...
function initResource(resource: string) {
    const url =
    ...
    fetch(url, {
        ...
        if (data.status >= 400) {
            throw new Error('Could not get calls info.');
        }
        ...
        for (var i = 0; i < tmp['result'].length; i++) {
            console.log(
                'Received result. resource: %s, result: %s',
                resource,
                JSON.stringify(tmp['result'][i])
            );
        }

        localStorage.setItem(resource, tmp['result']);
    });
}
...
function initResources() {
    // calls
    initResource('calls');
    initResource('conferences');
}

const authProvider: AuthProvider = {
    login: async ({ username, password }) => {
        console.log(
            'Check values. username: %s, password: %s',
            username,
            password
        );
