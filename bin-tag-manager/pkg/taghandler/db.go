...
// dbList returns tags
func (h *tagHandler) dbList(ctx context.Context, size uint64, token string, filters map[tag.Field]any) ([]*tag.Tag, error) {
	...
	res, err := h.db.TagList(ctx, size, token, filters)
	if err != nil {
