...
if "routing_stt" in sys.modules:
    del sys.modules["routing_stt"]
...
class TestSetActiveMember:
    ...
    def test_unknown_member_keeps_previous(self):
        """Unknown member preserves the previous active STT (safer than going deaf)."""
        services = _make_services()
        routing = RoutingSTTService(services)
        routing.set_active_member("member-a")
