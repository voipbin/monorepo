...
func Test_Run_sequencing(t *testing.T) {
	...
	queueName := string(commonoutline.QueueNameRegistrarSubscribe)
	...
	mockSock.EXPECT().ConsumeMessage(gomock.Any(), queueName, string(commonoutline.ServiceNameRegistrarManager), false, false, false, 10, gomock.Any()).Return(nil).AnyTimes()
