...
func Test_ChatGets(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			10,
			map[chat.Field]any{
				chat.FieldCustomerID:   uuid.FromStringOrNil("20db856a-051f-49d3-986e-3715af514273"),
				chat.FieldDeleted:      false,
				chat.FieldType:         chat.TypeNormal,
				chat.FieldRoomOwnerID:  uuid.FromStringOrNil("e90b7010-b936-11ee-8da8-bb61becf0b57"),
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(cs) != len(tt.data) {
				t.Errorf("Wrong length.\nexpect: %d\ngot: %d", len(tt.data), len(cs))
			}
			for _, expected := range tt.data {
				found := false
				for _, got := range cs {
