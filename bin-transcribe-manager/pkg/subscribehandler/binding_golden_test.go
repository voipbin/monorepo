...
// Test_topicPatterns_golden pins the EXACT bind set of transcribe-manager on the global
...
func Test_topicPatterns_golden(t *testing.T) {
	expected := []string{
		...
		"call-manager.confbridge.*.terminated",
		"customer-manager.customer.*.deleted",
	}

	// design §5: transcribe-manager binds exactly 3 patterns.
...
// Test_fanoutUnbindTargets_golden pins the fanout exchanges transcribe-manager unbinds
// after a fully successful topic bind (VOIP-1406). This is the FULL subscribeTargets set
// wired in cmd/transcribe-manager: the call-manager and customer-manager event exchanges.
