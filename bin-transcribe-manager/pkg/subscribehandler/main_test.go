...
func Test_Run_sequencing(t *testing.T) {
	...
	queueName := string(commonoutline.QueueNameTranscribeSubscribe)
	...
	mockSock.EXPECT().ConsumeMessage(gomock.Any(), queueName, string(common.Servicename), false, false, false, 10, gomock.Any()).Return(nil).AnyTimes()

	h := NewSubscribeHandler(
		mockSock,
		commonoutline.QueueNameTranscribeSubscribe,
		subscribeTargets,
		nil,
	)
