...
// and no runtime metric can detect it. Key schema: VOIP-1404 design §4.2 / §8.
//
...
//
// The parameter stays `any` (not the interface), matching the golden table's heterogeneous
// `data any` column; a non-implementing payload resolves to "" (→ placeholder).
...
// resolveSubscriptionID (and, by parity, in notifyhandler): a nil *transcript.Transcript still
// satisfies the SubscriptionIdentifier assertion, but calling the method would dereference a nil
// receiver. The guard must short-circuit to "", so the key degrades to the `-` placeholder
// instead of panicking. Removing the guard from the helper makes this test panic — it is the
// mutation lock for the guard itself, which no address-value row can provide.
