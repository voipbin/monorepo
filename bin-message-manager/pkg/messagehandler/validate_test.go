...
// Test_Send_unverified asserts that an unverified customer's send is rejected
// before the balance check (no BillingV1AccountIsValidBalanceByCustomerID call,
// no message persisted).
func Test_Send_unverified(t *testing.T) {
	...
	res, err := h.Send(ctx, id, customerID, nil, nil, "blocked")
	if err == nil {
