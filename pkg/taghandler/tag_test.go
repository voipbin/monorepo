...
func Test_Delete(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			responseAgents: []*agent.Agent{
				{
					ID: uuid.FromStringOrNil("bf52d90e-2a4b-11ee-84df-17815a4b7663"),
					TagIDs: []uuid.UUID{
						uuid.FromStringOrNil("a6b3cf48-2a4b-11ee-b574-2bad4f039ce5"),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockDB.EXPECT().TagGet(ctx, tt.id).Return(tt.responseTag, nil)
			mockAgent.EXPECT().GetsByTagIDs(ctx, tt.responseTag.CustomerID, []uuid.UUID{tt.id}).Return(tt.responseAgents, nil)
			for i, ag := range tt.responseAgents {
				mockAgent.EXPECT().UpdateTagIDs(ctx, ag.ID, tt.expectNewTagIds[i]).Return(&agent.Agent{}, nil)
			}
