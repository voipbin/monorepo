...
func (h *listenHandler) v1ChatroomsGet(ctx context.Context, m *rabbitmqhandler.Request) (*rabbitmqhandler.Response, error) {
	...
	var tmp []*chatroom.Chatroom
	if customerID != uuid.Nil {
		// gets by customer id
		tmp, err = h.chatroomHandler.GetsByCustomerID(ctx, customerID, pageToken, pageSize)
		if err != nil {
			log.Errorf("Could not get chats by GetsByCustomerID. err: %v", err)
			return nil, err
		}
	} else if ownerID != uuid.Nil {
		// gets by owner id
		tmp, err = h.chatroomHandler.GetsByOwnerID(ctx, ownerID, pageToken, pageSize)
		if err != nil {
		...
		return nil, fmt.Errorf("unsupported get option")
	}
