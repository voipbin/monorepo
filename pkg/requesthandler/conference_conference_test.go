...
func Test_ConferenceV1ConferenceAddConferencecallID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			uuid.FromStringOrNil("3e890a32-94a5-11ed-8e2c-77a93c20804c"),
			uuid.FromStringOrNil("3ec0cde6-94a5-11ed-8d2f-37e09e10a029"),

			&cfconference.Conference{
				ID: uuid.FromStringOrNil("3e890a32-94a5-11ed-8e2c-77a93c20804c"),
			},
			&rabbitmqhandler.Response{
				StatusCode: 200,
				DataType:   "application/json",
				Data:       []byte(`{"id":"3e890a32-94a5-11ed-8e2c-77a93c20804c"}`),
			},

			"bin-manager.conference-manager.request",
			&rabbitmqhandler.Request{
				URI:      "/v1/conferences/3e890a32-94a5-11ed-8e2c-77a93c20804c/conferencecall_ids",
				Method:   rabbitmqhandler.RequestMethodPost,
				DataType: "application/json",
				Data:     []byte(`{"conferencecall_id":"3ec0cde6-94a5-11ed-8d2f-37e09e10a029"}`),
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			res, err := reqHandler.ConferenceV1ConferenceAddConferencecallID(ctx, tt.conferenceID, tt.conferencecallID)
			if err != nil {
				t.Errorf("Wrong match. expect ok, got: %v", err)
			}
