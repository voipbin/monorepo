...
func (r *requestHandler) RMV1DomainDelete(ctx context.Context, domainID uuid.UUID) error {
	uri := fmt.Sprintf("/v1/domains/%s", domainID)

	res, err := r.sendRequestRM(uri, rabbitmqhandler.RequestMethodDelete, resourceRegistrarDomains, requestTimeoutDefault, 0, ContentTypeJSON, nil)
	switch {
	case err != nil:
		return err
	case res == nil:
