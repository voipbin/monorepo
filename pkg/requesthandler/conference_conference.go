...
// ConferenceV1ConferenceAddConferencecallID sends a request to conference-manager
// to add the conferencecall_id from the conference.
// it returns error if it failed.
func (r *requestHandler) ConferenceV1ConferenceAddConferencecallID(ctx context.Context, conferenceID uuid.UUID, conferencecallID uuid.UUID) (*cfconference.Conference, error) {
	uri := fmt.Sprintf("/v1/conferences/%s/conferencecall_ids", conferenceID)

	data := &cfrequest.V1DataConferencesIDConferencecallIDsPost{
