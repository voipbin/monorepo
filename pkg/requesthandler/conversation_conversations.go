...
func (r *requestHandler) ConversationV1ConversationGet(ctx context.Context, conversationID uuid.UUID) (*cvconversation.Conversation, error) {
	...
	tmp, err := r.sendRequestConversation(uri, rabbitmqhandler.RequestMethodGet, resourceConversationConversations, requestTimeoutDefault, 0, ContentTypeJSON, nil)
	if err != nil {
		return nil, err
	}
...
func (r *requestHandler) ConversationV1MessageSend(ctx context.Context, conversationID uuid.UUID, text string, medias []cvmedia.Media) (*cvmessage.Message, error) {
	...
	tmp, err := r.sendRequestConversation(uri, rabbitmqhandler.RequestMethodPost, resourceConversationConversationsIDMessages, requestTimeoutDefault, 0, ContentTypeJSON, m)
	switch {
	case err != nil:
		return nil, err
	case tmp == nil:
