package dbhandler

import (
	"context"
	"fmt"
	"reflect"
	"testing"

	uuid "github.com/gofrs/uuid"
	gomock "github.com/golang/mock/gomock"

	"gitlab.com/voipbin/bin-manager/chat-manager.git/models/chat"
	"gitlab.com/voipbin/bin-manager/chat-manager.git/pkg/cachehandler"
)

func Test_ChatCreate(t *testing.T) {

	tests := []struct {
		name string

		chat *chat.Chat
	}{
		{
			"empty all",

			&chat.Chat{
				ID:       uuid.FromStringOrNil("6127ed80-8f71-4f93-883c-bbeab83f0a10"),
				TMDelete: DefaultTimeStamp,
			},
		},
		{
			"all items",

			&chat.Chat{
				ID:         uuid.FromStringOrNil("d649c8b5-c5f6-4740-a28c-653a47195a1d"),
				CustomerID: uuid.FromStringOrNil("4dbf893c-82fe-4dea-8079-272670aea7b4"),
				Type:       chat.TypeNormal,
				OwnerID:    uuid.FromStringOrNil("c4111534-e222-4e0a-9485-f012ea0a9e02"),
				ParticipantIDs: []uuid.UUID{
					uuid.FromStringOrNil("3beef418-cf3a-4239-adff-6f096cebacae"),
					uuid.FromStringOrNil("47adb891-80aa-4ba4-af47-446faa6cf386"),
				},
				Name:     "test name",
				Detail:   "test detail",
				TMCreate: "2020-04-18T03:22:17.995000",
				TMUpdate: "2020-04-18T03:22:17.995000",
				TMDelete: DefaultTimeStamp,
			},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mc := gomock.NewController(t)
			defer mc.Finish()

			mockCache := cachehandler.NewMockCacheHandler(mc)
			h := handler{
				db:    dbTest,
				cache: mockCache,
			}

			ctx := context.Background()

			mockCache.EXPECT().ChatSet(ctx, gomock.Any())
			if err := h.ChatCreate(ctx, tt.chat); err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			mockCache.EXPECT().ChatGet(ctx, tt.chat.ID).Return(nil, fmt.Errorf(""))
			mockCache.EXPECT().ChatSet(ctx, gomock.Any())
			res, err := h.ChatGet(ctx, tt.chat.ID)
			if err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}
			if reflect.DeepEqual(tt.chat, res) == false {
				t.Errorf("Wrong match.\nexpect: %v\ngot: %v", tt.chat, res)
			}

			resChatByParticipants, err := h.ChatGetByTypeAndParticipantsID(ctx, tt.chat.CustomerID, tt.chat.Type, tt.chat.ParticipantIDs)
			if err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}
			if reflect.DeepEqual(tt.chat, resChatByParticipants) == false {
				t.Errorf("Wrong match.\nexpect: %v\ngot: %v", tt.chat, resChatByParticipants)
			}
		})
	}
}

func Test_ChatGetsByCustomerID(t *testing.T) {

	tests := []struct {
		name string

		customerID uuid.UUID
		limit      uint64

		chats []*chat.Chat
	}{
		{
			"normal",
			uuid.FromStringOrNil("20db856a-051f-49d3-986e-3715af514273"),
			10,
			[]*chat.Chat{
				{
					ID:         uuid.FromStringOrNil("837117d8-0c31-11eb-9f9e-6b4ac01a7e66"),
					CustomerID: uuid.FromStringOrNil("20db856a-051f-49d3-986e-3715af514273"),
					TMCreate:   "2020-04-19T03:22:17.995000",
					TMDelete:   DefaultTimeStamp,
				},
				{
					ID:         uuid.FromStringOrNil("845e04f8-0c31-11eb-a8cf-6f8836b86b2b"),
					CustomerID: uuid.FromStringOrNil("20db856a-051f-49d3-986e-3715af514273"),
					TMCreate:   "2020-04-18T03:22:17.995000",
					TMDelete:   DefaultTimeStamp,
				},
			},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mc := gomock.NewController(t)
			defer mc.Finish()

			mockCache := cachehandler.NewMockCacheHandler(mc)
			h := handler{
				db:    dbTest,
				cache: mockCache,
			}

			ctx := context.Background()

			for _, c := range tt.chats {
				mockCache.EXPECT().ChatSet(ctx, gomock.Any())
				if err := h.ChatCreate(ctx, c); err != nil {
					t.Errorf("Wrong match. expect: ok, got: %v", err)
				}
			}

			cs, err := h.ChatGetsByCustomerID(ctx, tt.customerID, GetCurTime(), tt.limit)
			if err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			if reflect.DeepEqual(cs, tt.chats) != true {
				t.Errorf("Wrong match.\nexpect: %v\ngot: %v", tt.chats, cs)
			}
		})
	}
}

func Test_ChatGetsByType(t *testing.T) {

	tests := []struct {
		name       string
		customerID uuid.UUID
		chatType   chat.Type
		limit      uint64
		chats      []*chat.Chat
	}{
		{
			"type single",
			uuid.FromStringOrNil("12437b9d-4271-491f-babf-21d2c8aa5081"),
			chat.TypeNormal,
			10,
			[]*chat.Chat{
				{
					ID:         uuid.FromStringOrNil("acfa013e-78ce-4505-9b0d-8b011f33d0fc"),
					CustomerID: uuid.FromStringOrNil("12437b9d-4271-491f-babf-21d2c8aa5081"),
					Type:       chat.TypeNormal,
					TMCreate:   "2020-04-19T03:22:17.995000",
					TMDelete:   DefaultTimeStamp,
				},
				{
					ID:         uuid.FromStringOrNil("c8be2358-75b3-452b-8578-3ed86531f518"),
					CustomerID: uuid.FromStringOrNil("12437b9d-4271-491f-babf-21d2c8aa5081"),
					Type:       chat.TypeNormal,
					TMCreate:   "2020-04-18T03:22:17.995000",
					TMDelete:   DefaultTimeStamp,
				},
			},
		},
		{
			"type group",
			uuid.FromStringOrNil("0cfb4a33-0f80-46f5-b479-9a91bdd3ab73"),
			chat.TypeGroup,
			10,
			[]*chat.Chat{
				{
					ID:         uuid.FromStringOrNil("82153673-1865-4045-acd3-f2c6191a706f"),
					CustomerID: uuid.FromStringOrNil("0cfb4a33-0f80-46f5-b479-9a91bdd3ab73"),
					Type:       chat.TypeGroup,
					TMCreate:   "2020-04-19T03:22:17.995000",
					TMDelete:   DefaultTimeStamp,
				},
				{
					ID:         uuid.FromStringOrNil("b0e1bbf4-740a-4bc1-a33b-233eae403bc4"),
					CustomerID: uuid.FromStringOrNil("0cfb4a33-0f80-46f5-b479-9a91bdd3ab73"),
					Type:       chat.TypeGroup,
					TMCreate:   "2020-04-18T03:22:17.995000",
					TMDelete:   DefaultTimeStamp,
				},
			},
		}}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mc := gomock.NewController(t)
			defer mc.Finish()

			mockCache := cachehandler.NewMockCacheHandler(mc)
			h := handler{
				db:    dbTest,
				cache: mockCache,
			}

			ctx := context.Background()

			for _, c := range tt.chats {
				mockCache.EXPECT().ChatSet(ctx, gomock.Any())
				if err := h.ChatCreate(ctx, c); err != nil {
					t.Errorf("Wrong match. expect: ok, got: %v", err)
				}
			}

			res, err := h.ChatGetsByType(ctx, tt.customerID, tt.chatType, GetCurTime(), tt.limit)
			if err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			if reflect.DeepEqual(res, tt.chats) != true {
				t.Errorf("Wrong match.\nexpect: %v\ngot: %v", tt.chats, res)
			}
		})
	}
}

func Test_ChatUpdateBasic(t *testing.T) {

	tests := []struct {
		name string
		chat *chat.Chat

		id       uuid.UUID
		chatName string
		detail   string

		expectRes *chat.Chat
	}{
		{
			"normal",
			&chat.Chat{
				ID: uuid.FromStringOrNil("599b43c2-bb10-45fb-a4f4-00e04ecf4c28"),
			},

			uuid.FromStringOrNil("599b43c2-bb10-45fb-a4f4-00e04ecf4c28"),
			"update name",
			"update detail",

			&chat.Chat{
				ID:     uuid.FromStringOrNil("599b43c2-bb10-45fb-a4f4-00e04ecf4c28"),
				Name:   "update name",
				Detail: "update detail",
			},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mc := gomock.NewController(t)
			defer mc.Finish()

			mockCache := cachehandler.NewMockCacheHandler(mc)
			h := NewHandler(dbTest, mockCache)

			ctx := context.Background()

			mockCache.EXPECT().ChatSet(ctx, gomock.Any())
			if err := h.ChatCreate(ctx, tt.chat); err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			mockCache.EXPECT().ChatSet(ctx, gomock.Any())
			if err := h.ChatUpdateBasicInfo(ctx, tt.id, tt.chatName, tt.detail); err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			mockCache.EXPECT().ChatGet(ctx, tt.chat.ID).Return(nil, fmt.Errorf(""))
			mockCache.EXPECT().ChatSet(ctx, gomock.Any())
			res, err := h.ChatGet(ctx, tt.id)
			if err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			res.TMUpdate = ""
			res.TMCreate = ""
			if reflect.DeepEqual(tt.expectRes, res) == false {
				t.Errorf("Wrong match.\nexpect: %v\ngot: %v", tt.expectRes, res)
			}
		})
	}
}

func Test_ChatDelete(t *testing.T) {
	tests := []struct {
		name string

		id uuid.UUID

		chat *chat.Chat
	}{
		{
			"normal",

			uuid.FromStringOrNil("66edc48d-d2c0-475f-ad10-9e2fffec8626"),

			&chat.Chat{
				ID: uuid.FromStringOrNil("66edc48d-d2c0-475f-ad10-9e2fffec8626"),
			},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mc := gomock.NewController(t)
			defer mc.Finish()

			mockCache := cachehandler.NewMockCacheHandler(mc)

			h := &handler{
				db:    dbTest,
				cache: mockCache,
			}

			ctx := context.Background()

			mockCache.EXPECT().ChatSet(ctx, gomock.Any())
			if err := h.ChatCreate(ctx, tt.chat); err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			mockCache.EXPECT().ChatSet(ctx, gomock.Any())
			if err := h.ChatDelete(ctx, tt.id); err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			mockCache.EXPECT().ChatGet(ctx, tt.id).Return(nil, fmt.Errorf("error"))
			mockCache.EXPECT().ChatSet(ctx, gomock.Any()).Return(nil)
			res, err := h.ChatGet(ctx, tt.id)
			if err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			if res.TMDelete == DefaultTimeStamp {
				t.Errorf("Wrong match. expect: any other, got: %s", res.TMDelete)
			}
		})
	}
}

func Test_ChatUpdateOwnerID(t *testing.T) {

	tests := []struct {
		name string
		chat *chat.Chat

		id      uuid.UUID
		ownerID uuid.UUID

		expectRes *chat.Chat
	}{
		{
			"normal",
			&chat.Chat{
				ID:      uuid.FromStringOrNil("48703865-5b0f-4d82-b7de-9267e040996b"),
				Name:    "name",
				Detail:  "detail",
				OwnerID: uuid.FromStringOrNil("0497e2dc-4af5-4baa-a986-8ca50bf001fb"),
			},

			uuid.FromStringOrNil("48703865-5b0f-4d82-b7de-9267e040996b"),
			uuid.FromStringOrNil("4b25fe52-02c7-4201-9fa8-91b1bfce068e"),

			&chat.Chat{
				ID:      uuid.FromStringOrNil("48703865-5b0f-4d82-b7de-9267e040996b"),
				Name:    "name",
				Detail:  "detail",
				OwnerID: uuid.FromStringOrNil("4b25fe52-02c7-4201-9fa8-91b1bfce068e"),
			},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			mc := gomock.NewController(t)
			defer mc.Finish()

			mockCache := cachehandler.NewMockCacheHandler(mc)
			h := NewHandler(dbTest, mockCache)

			ctx := context.Background()

			mockCache.EXPECT().ChatSet(ctx, gomock.Any())
			if err := h.ChatCreate(ctx, tt.chat); err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			mockCache.EXPECT().ChatSet(ctx, gomock.Any())
			if err := h.ChatUpdateOwnerID(ctx, tt.id, tt.ownerID); err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			mockCache.EXPECT().ChatGet(ctx, tt.id).Return(nil, fmt.Errorf(""))
			mockCache.EXPECT().ChatSet(ctx, gomock.Any())
			res, err := h.ChatGet(ctx, tt.id)
			if err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}

			res.TMUpdate = ""
			res.TMCreate = ""
			if reflect.DeepEqual(tt.expectRes, res) == false {
				t.Errorf("Wrong match.\nexpect: %v\ngot: %v", tt.expectRes, res)
			}
		})
	}
}
