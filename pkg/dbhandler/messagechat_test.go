...
func Test_MessagechatGetsByCustomerID(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"normal",
			uuid.FromStringOrNil("7d06eada-20ac-11ed-814e-174667679bd6"),
			10,
			[]*messagechat.Messagechat{
				{
					ID:         uuid.FromStringOrNil("7d6e8d7a-20ac-11ed-8e0a-d305cf639cca"),
					CustomerID: uuid.FromStringOrNil("7d06eada-20ac-11ed-814e-174667679bd6"),
					TMCreate:   "2020-04-19T03:22:17.995000",
					TMDelete:   DefaultTimeStamp,
				},
				{
					ID:         uuid.FromStringOrNil("7d43e80e-20ac-11ed-a248-371e83cfc0d4"),
					CustomerID: uuid.FromStringOrNil("7d06eada-20ac-11ed-814e-174667679bd6"),
					TMCreate:   "2020-04-18T03:22:17.995000",
					TMDelete:   DefaultTimeStamp,
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cs, err := h.MessagechatGetsByCustomerID(ctx, tt.customerID, GetCurTime(), tt.limit)
			if err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}
...
func Test_MessagechatGetsByChatID(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cs, err := h.MessagechatGetsByChatID(ctx, tt.chatID, GetCurTime(), tt.limit)
			if err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}
