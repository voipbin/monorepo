...
func Test_BillingEndByReferenceID(t *testing.T) {
	...
	tests := []test{
		{
			...
			responseBilling: &billing.Billing{
				ID:             uuid.FromStringOrNil("a27635f2-09f7-11ee-8003-8f7d80359ed0"),
				AccountID:      uuid.FromStringOrNil("01592262-09fa-11ee-bf7d-b34a889017cc"),
				ReferenceType:  billing.ReferenceTypeCall,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := h.BillingEndByReferenceID(ctx, tt.referenceID, tt.tmBillingEnd, tt.source, tt.destination); err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}
		})
	}
