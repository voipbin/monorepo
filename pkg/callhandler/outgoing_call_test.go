...
func Test_createCallsOutgoingGroupdial_endpoint(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			responseUUIDID: uuid.FromStringOrNil("08701bca-b5e8-11ed-9257-4bee6cbc72bf"),
			responseUUIDs: []uuid.UUID{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(resCalls, tt.responseCalls) {
				t.Errorf("Wrong match.\nexpect: %v\ngot: %v", tt.responseCalls, resCalls)
			}
		})
	}
...
func Test_getDialDestinations_getDialDestinationsAddressTypeAgent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			res, err := h.getDialDestinations(ctx, tt.customerID, tt.destination)
			if err != nil {
				t.Errorf("Wrong match. expect: ok, got: %v", err)
			}
