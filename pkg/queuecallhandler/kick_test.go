...
func Test_Kick(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockDB.EXPECT().QueuecallSetStatusKicking(ctx, tt.queuecallID).Return(nil)
			mockDB.EXPECT().QueuecallGet(ctx, tt.queuecallID).Return(tt.responseQueuecall, nil)
			mockNotify.EXPECT().PublishWebhookEvent(ctx, tt.responseQueuecall.CustomerID, queuecall.EventTypeQueuecallKicking, tt.responseQueuecall)
...
func Test_KickByReferenceID(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			&queuecallreference.QueuecallReference{
				...
				CurrentQueuecallID: uuid.FromStringOrNil("35668acc-d1b6-11ec-951d-4b7146227c1b"),
			},
			&queuecall.Queuecall{
				ID:              uuid.FromStringOrNil("101b66d0-d1b5-11ec-b1fc-03d1a45f37e3"),
				QueueID:         uuid.FromStringOrNil("c935c7d0-5edf-11ec-8d87-5b567f32807e"),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockReq.EXPECT().FMV1ActiveflowUpdateForwardActionID(ctx, tt.responseQueuecall.ReferenceID, tt.responseQueuecall.ExitActionID, true).Return(nil)
			mockDB.EXPECT().QueuecallSetStatusKicking(ctx, tt.responseQueuecallReference.CurrentQueuecallID).Return(nil)
			mockDB.EXPECT().QueuecallGet(ctx, tt.responseQueuecallReference.CurrentQueuecallID).Return(tt.responseQueuecall, nil)
			mockNotify.EXPECT().PublishWebhookEvent(ctx, tt.responseQueuecall.CustomerID, queuecall.EventTypeQueuecallKicking, tt.responseQueuecall)
