...
| Pod annotation patch failing — **process exits during startup, not a degraded state** | Not running in Kubernetes; service account lacks `patch` permission on pods | `setProxyInfoAnnotation`'s error return makes `main()` return before the RabbitMQ/ARI/AMI handlers are ever constructed, so this is a **silent startup abort** (clean exit, not a crash) rather than the proxy merely running without the annotation set. Set `--kubernetes_disabled=true` for non-k8s environments; grant `patch` verb on pods to service account |
| Recording upload fails | GCS bucket not configured or credentials not available | Set `--recording_bucket_name`; provide credentials via `GOOGLE_APPLICATION_CREDENTIALS_JSON` (service-account key JSON) or ensure ADC/workload-identity grants `roles/storage.objectCreator` on the bucket |
...
| `--google_application_credentials_json` | `GOOGLE_APPLICATION_CREDENTIALS_JSON` | `` | GCP service-account key JSON (content, not a path) used to authenticate GCS recording uploads. When empty, Application Default Credentials (ADC) are used. Required on bare-metal where no workload-identity ADC is available; if unset there, recording upload is skipped with a clear error instead of crashing the proxy |
