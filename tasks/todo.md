...
- [ ] `bin-api-manager/models/auth/auth.go` `DirectScope`(`:35-40`)에 5필드 추가
...
- [ ] `bin-api-manager/pkg/servicehandler/boot.go` `AuthBoot`의 `scope` 생성부(`:121`)
...
- [ ] 상수: `BootSessionMaxLifetime = time.Hour * 24`, `DirectScopeVersionIssued = 1`, `DirectScopeVersionEnforced = 2`
      **1단계에서 `AuthBoot`은 `DirectScopeVersionIssued`를 참조한다.** 3단계에서 참조를 바꾼다(상수 값을 바꾸지 않는다)
- [ ] `directHashFingerprint(hash string) string`: `sha256.Sum256([]byte(hash))`의 hex 전체. 절단·솔트 금지(설계 §3.6).
      **`boot.go`에 둔다.** §2.3의 `boot_refresh.go`도 같은 패키지에서 호출한다. 중복 정의하지 말 것
- [ ] `BootResponse`(`:27-46`)에 **두 필드** 추가하고 응답에 채운다
...
- [ ] `bin-api-manager/pkg/servicehandler/boot_refresh.go` 신설: `AuthBootRefresh(ctx, a *auth.AuthIdentity) (*BootResponse, error)`
      설계 §3.6의 7단계를 그 순서대로:
...
- [ ] **만료는 duration 쪽에서 자른다.** `authJWTGenerateWithExpiration`은 절대 시각을 받지 않는다(호출부 `boot.go:133`)
...
- [ ] 응답은 `BootResponse`에서 `ResourceData`만 뺀 형태. 나머지 필드는 전부 채운다
- [ ] `ServiceHandler` 인터페이스(`pkg/servicehandler/main.go`) 추가 + mock 재생성
- [ ] `bin-api-manager/lib/service/boot.go`에 `PostBootRefresh` 신설.
...
- [ ] `bin-api-manager/cmd/api-manager/main.go` `authProtected` 그룹(`:306-312`)에 `POST /boot/refresh` 등록

### 2.4 id 배관: aicall (설계 §4.2.1, 전부 inert)

- [ ] `bin-common-handler/pkg/requesthandler/ai_aicalls.go:18` `AIV1AIcallStart`에 `id uuid.UUID` + 인터페이스(`main.go:283`) + mock
- [ ] `bin-api-manager/pkg/servicehandler/aicall.go:93` 호출부 `uuid.Nil`
- [ ] `bin-api-manager/pkg/servicehandler/serviceagent_aicall.go:284` 호출부 `uuid.Nil`
- [ ] `bin-ai-manager/pkg/listenhandler/models/request/aicalls.go`에 `ID uuid.UUID \`json:"id,omitempty"\`` 추가
      **`omitempty`는 `[16]byte`에 효과가 없어 항상 zero UUID로 직렬화된다.** 무해하며 transcribe 선례와 동일. 나중에 "고치지" 말 것
- [ ] `bin-ai-manager/pkg/listenhandler/v1_aicalls.go:91` `req.ID` 전달
- [ ] `bin-ai-manager/pkg/aicallhandler/start.go:170` `Start`에 `id` + 인터페이스 + mock
- [ ] `bin-ai-manager/pkg/aicallhandler/service.go:54`, `:96` 호출부 `uuid.Nil`
- [ ] `startReferenceType*` 4개(`:204/:245/:405/:606`), `startAIcallByRealtime`(`:993`), `startAIcallByMessaging`(`:1050`)에 `id` 관통
- [ ] `startAIcallByMessaging` 호출 4곳(`:286/:424/:622/:1121`), `startAIcallByRealtime` 호출 1곳(`:228`)
- [ ] `bin-ai-manager/pkg/aicallhandler/db.go:41`, `:115`: `id == uuid.Nil`이면 `UUIDCreate()`, 아니면 그대로

### 2.5 id 배관: webchat session (호출자 조사 완료)

- [ ] `bin-common-handler/pkg/requesthandler/webchat_session.go:17` `WebchatV1SessionCreate`에 `id` + 인터페이스(`main.go:1567`) + mock
- [ ] `bin-api-manager/pkg/servicehandler/webchat_session.go:217` 호출부 `uuid.Nil`
- [ ] `bin-webchat-manager/pkg/listenhandler/models/request` `V1DataSessionsPost`에 `ID` 추가
- [ ] `bin-webchat-manager/pkg/listenhandler/v1_sessions.go:33` `req.ID` 전달
- [ ] `bin-webchat-manager/pkg/sessionhandler/create.go:29` `Create`에 `id` + 인터페이스 + mock
- [ ] `bin-webchat-manager/pkg/sessionhandler/create.go:37`: `id == uuid.Nil`이면 `UUIDCreate()`, 아니면 그대로

### 2.6 409 경로 (설계 §4.2.2)

**aicall:**

- [ ] **`bin-ai-manager/pkg/dbhandler/aicall.go:38`의 `%v` 래핑을 유지한다.**
      `IsErrDuplicate`의 기존 소비자 2곳(`aicallhandler/start.go:444`, `aihandler/db.go:229`)이 문자열 매칭에 의존한다
- [ ] 분류는 `aicallhandler.Start`(`start.go:170-201`)에서 **switch 이후 반환값**에 대해,
...
- [ ] `errAIcallIDAlreadyExists`: 드라이버 에러를 `%w`/`%v`로 감싸지 않은 새 `cerrors.AlreadyExists`

**webchat session: 판별 수단부터 만들어야 한다:**

- [ ] **`bin-webchat-manager`에는 중복 판별 수단이 없다.** `IsErrDuplicate` 0건, db 센티널은 `ErrNotFound` 하나뿐(`pkg/dbhandler/main.go:52`).
      그리고 `errorResponse`(`pkg/listenhandler/main.go:102-123`)는 타입 없는 에러를 **500**으로 떨어뜨린다.
      아무것도 안 하면 **webchat 위젯의 계기 2가 죽는다.**
- [ ] `bin-webchat-manager/pkg/dbhandler/main.go`에 `IsErrDuplicate` 신설.
...
- [ ] **`SessionGet` 기반 사전 확인을 쓰지 말 것.** §1.1이 금지하며, transcribe 선례를 따라가면 설계가 명시적으로 거부한 교차 테넌트 존재 오라클을 재도입하게 된다
- [ ] `sessionhandler.Create`에서 `callerSpecifiedID && IsErrDuplicate(err)`일 때 `cerrors.AlreadyExists` 반환.
...
- [ ] `bin-openapi-manager/openapi/paths/auth/boot-refresh.yaml` 신설. 기존 6개 `paths/auth/*.yaml` 형식을 따른다.
...
- [ ] `bin-openapi-manager/openapi/openapi.yaml`의 paths에 `/auth/boot/refresh` 엔트리 추가(`:8416` 인근)
- [ ] `AuthBootResponse`(`openapi.yaml:8156`)에 `allowed_resource_id`, `scope_version` 추가
- [ ] `bin-openapi-manager`에서 `go generate ./...` → `gens/models/gen.go` 커밋 → 이어서 `bin-api-manager`
- [ ] **`bin-api-manager/server/auth_boot_refresh.go` 신설.** `server/auth_boot.go:13-19` 형태의 stub으로
...
- [ ] `bin-api-manager/server/auth_stubs_test.go:24` 표에 행 추가

**RST:**

- [ ] `POST /auth/boot/refresh`, `allowed_resource_id`, `scope_version`을 `bin-api-manager/docsdev/source/`에 반영
- [ ] 클린 빌드: `cd bin-api-manager/docsdev && rm -rf build && python3 -m sphinx -M html source build` 후 `git add -f bin-api-manager/docsdev/build/`

**서비스 docs (존재하는 파일만. rev2는 없는 파일 2개를 지목했다):**

- [ ] `bin-api-manager/docs/routing.md`: 새 라우트
- [ ] `bin-api-manager/docs/architecture.md`: `cmd/api-manager/main.go` 변경분
- [ ] `bin-api-manager/docs/auth.md`: `DirectScope` 필드와 갱신 절차
      ...
      해당 내용은 `auth.md`로 간다)
- [ ] `bin-ai-manager/docs/architecture.md`: 요청 DTO 필드 추가분
...
- [ ] `AuthBoot`이 5필드를 채우는지. `AllowedResourceID`가 매 부팅마다 다른지. `BootExpire`가 부팅+24h인지. `ScopeVersion == 1`인지
- [ ] `BootResponse`에 `allowed_resource_id`와 **`scope_version`**이 실려 나가는지
- [ ] **부팅 응답의 `allowed_resource_id`·`scope_version`이 발급된 JWT의 `direct` 클레임 값과 동일한지.**
      단일 출처 규칙(§2.2)의 회귀 고정
- [ ] 갱신 검사 1: agent/accesskey/delegate 각각 403이고 **패닉·500이 아닌지**
- [ ] 갱신: **인증 없이는 거부되는지**
- [ ] 갱신: **본문에 `allowed_resource_id`를 넣어도 무시되는지**
- [ ] 갱신 검사 2: `AllowedResourceID`/`DirectID` Nil, `HashFingerprint` 빈 문자열 각각 401
- [ ] 갱신 검사 3: `BootExpire` 경과 시 401. 파싱 실패 시 401
- [ ] 갱신 검사 4: direct 레코드 삭제 시 401. **RPC 오류일 때도 401**
- [ ] 갱신 검사 5: **해시 재발급 후 기존 토큰의 갱신이 401** (무효화가 갱신을 이기는지, 핵심 케이스)
- [ ] 갱신 검사 6: 동결·삭제 고객사 401. **RPC 오류일 때도 401**
- [ ] 갱신 7단계: `directResourceMapping` 미스 시 401
- [ ] 갱신 7단계: **2회 연속 갱신 후 9필드가 `expire` 외 전부 동일**
- [ ] 갱신 후 토큰으로 **WS 토픽 구독이 여전히 통과하는지** (`CustomerID` 유실의 행위적 탐지기)
- [ ] 갱신: `BootExpire` 직전 갱신의 `expire`가 상한을 넘지 않는지. **duration 쪽에서 잘렸는지**
- [ ] 갱신: `ScopeVersion`이 증가하지 않고 복사되는지
- [ ] 갱신 응답에 `resource_data`가 없고 나머지 `BootResponse` 필드는 전부 있는지
- [ ] 배관: 기존 호출자 전부가 `uuid.Nil`로 서버 생성 경로를 그대로 타는지
- [ ] 배관: 지정 id가 실제로 그 id로 저장되는지 (aicall·webchat 양쪽)
- [ ] 409: aicall·**webchat 양쪽**에서 PK 충돌이 409로 RPC 경계를 넘어오는지
- [ ] **`IsErrDuplicate` 비회귀**: `aicallhandler/start.go:444`와 `aihandler/db.go:229`가 그대로 동작
- [ ] 센티널이 드라이버 에러를 감싸지 않아 `IsErrDuplicate`에 걸리지 않는지
- [ ] 시그니처 변경으로 깨지는 기존 테스트 수정: `requesthandler/ai_aicalls_test.go:82`,
...
- [ ] **`bin-api-manager/pkg/servicehandler/boot_test.go` 수정.** 시그니처 변경이 아니라 mock 기대값 문제다.
...
- [ ] 5개 디렉토리 전부 통과. `go.mod`/`go.sum` 변경분을 함께 커밋
...
- [ ] VOIP-1501·VOIP-1498에 3단계 착륙 계획 코멘트
- [ ] 2단계용 신규 티켓(monorepo-javascript) 생성
- [ ] 3단계용 신규 티켓 생성
...
- **PR 분할 승인.** 필요한 승인은 **monorepo 안의 1단계/3단계 분할** 하나다.
  monorepo와 monorepo-javascript로 나뉘는 것은 저장소가 달라 구조적으로 불가피하며 승인 대상이 아니다.
  **이 게이트는 3단계 PR이 아니라 1단계 착수를 막는다**(전체 단계 구성이 여기에 의존한다).
  분할하지 않는 유일한 대안은 4시간 장애를 감수하는 것이다
