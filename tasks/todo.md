...
- [ ] 3.3 Follow-up Jira tickets (1405 §7; summary EN, body KR): talk-control defect (+post-fix option), dead NotifyHandlers (transfer + tts ttshandler), conversation secret stripping (PRIORITY), stream-completeness audit (incl. timeline subscribing never-published conversation_deleted), **legacy coverage uplift** for the sub-80% packages listed in AC7 (aggressive-coverage principle applied head-on in its own ticket).
