...
// processV1CustomersIDDefaultOutgoingSourceNumberIDPut handles Put /v1/customers/<customer-id>/default_outgoing_source_number_id request
func (h *listenHandler) processV1CustomersIDDefaultOutgoingSourceNumberIDPut(ctx context.Context, m *sock.Request) (*sock.Response, error) {
	uriItems := strings.Split(m.URI, "/")
	if len(uriItems) < 4 {
		return simpleResponse(400), nil
	}
	...
	id := uuid.FromStringOrNil(uriItems[3])
	log := logrus.WithFields(logrus.Fields{
		"func":        "processV1CustomersIDDefaultOutgoingSourceNumberIDPut",
		"customer_id": id,
	})
	log.Debug("Executing processV1CustomersIDDefaultOutgoingSourceNumberIDPut.")

	var req request.V1DataCustomersIDDefaultOutgoingSourceNumberIDPut
	if err := json.Unmarshal([]byte(m.Data), &req); err != nil {
		log.Debugf("Could not unmarshal the data. data: %v, err: %v", m.Data, err)
		return simpleResponse(400), nil
	}

	tmp, err := h.customerHandler.UpdateDefaultOutgoingSourceNumberID(ctx, id, req.DefaultOutgoingSourceNumberID)
	if err != nil {
