...
func Test_Run_sequencing(t *testing.T) {
	...
	queueName := string(commonoutline.QueueNameStorageSubscribe)
	...
	mockSock.EXPECT().ConsumeMessage(gomock.Any(), queueName, string(commonoutline.ServiceNameStorageManager), false, false, false, 10, gomock.Any()).Return(nil).AnyTimes()
