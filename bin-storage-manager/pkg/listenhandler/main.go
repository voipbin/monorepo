...
func (h *listenHandler) Run(queue, exchangeDelay string) error {
	...
	// receive requests
	go func() {
		if errConsume := h.sockHandler.ConsumeRPC(context.Background(), queue, "storage-manager", false, false, false, 10, h.processRequest); errConsume != nil {
			logrus.Errorf("Could not consume the message correctly. err: %v", errConsume)
		}
	}()
