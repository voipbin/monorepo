...
func Test_errorResponse_typed(t *testing.T) {
	resp := errorResponse(cerrors.NotFound(commonoutline.ServiceNameScheduleManager, "SCHEDULE_NOT_FOUND", "x"))
...
func Test_errorResponse_typedFailedPrecondition(t *testing.T) {
	resp := errorResponse(cerrors.FailedPrecondition(commonoutline.ServiceNameScheduleManager, "EXECUTION_IN_PROGRESS", "x"))
...
func Test_errorResponse_typedAlreadyExists(t *testing.T) {
	if errorResponse(cerrors.AlreadyExists(commonoutline.ServiceNameScheduleManager, "SCHEDULE_NAME_DUPLICATED", "x")).StatusCode != http.StatusConflict {
		t.Errorf("already exists failed")
	}
}

func Test_errorResponse_typedThroughPkgErrorsWrap(t *testing.T) {
	if errorResponse(pkgerrors.Wrap(cerrors.NotFound(commonoutline.ServiceNameScheduleManager, "x", "x"), "outer")).StatusCode != http.StatusNotFound {
...
func Test_errorResponse_typedNotFoundFromBusinessHandler(t *testing.T) {
	typed := cerrors.NotFound(commonoutline.ServiceNameScheduleManager, "SCHEDULE_NOT_FOUND", "x").Wrap(dbhandler.ErrNotFound)
