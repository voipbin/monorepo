...
// Column-width bounds (must match the schedule_schedules DDL).
const (
	maxLenName           = 255
	maxLenDetail         = 1024
...
func validateBounds(name, detail, targetURI, targetDataType string, targetData json.RawMessage) error {
	...
	case len(name) > maxLenName:
		return cerrors.InvalidArgument(commonoutline.ServiceNameScheduleManager, "SCHEDULE_NAME_TOO_LONG", "The schedule name must be at most 255 characters.")
	case len(detail) > maxLenDetail:
		return cerrors.InvalidArgument(commonoutline.ServiceNameScheduleManager, "SCHEDULE_DETAIL_TOO_LONG", "The schedule detail must be at most 1024 characters.")
	case targetURI == "":
		return cerrors.InvalidArgument(commonoutline.ServiceNameScheduleManager, "SCHEDULE_TARGET_URI_EMPTY", "The target uri must not be empty.")
	case len(targetURI) > maxLenTargetURI:
		return cerrors.InvalidArgument(commonoutline.ServiceNameScheduleManager, "SCHEDULE_TARGET_URI_TOO_LONG", "The target uri must be at most 1024 characters.")
	case len(targetDataType) > maxLenTargetDataType:
		return cerrors.InvalidArgument(commonoutline.ServiceNameScheduleManager, "SCHEDULE_TARGET_DATA_TYPE_TOO_LONG", "The target data type must be at most 255 characters.")
	case len(targetData) > 0 && !json.Valid(targetData):
		return cerrors.InvalidArgument(commonoutline.ServiceNameScheduleManager, "SCHEDULE_TARGET_DATA_INVALID", "The target data must be valid JSON.")
	}
	return nil
}
