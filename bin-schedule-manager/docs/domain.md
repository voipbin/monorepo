# bin-schedule-manager Domain

## Domain Entities

### Schedule

A named cron entry owned by the platform (nil `customer_id` in Phase 1; the schema already carries `customer_id` for Phase 3 multi-tenancy). Stored in `schedule_schedules`.
...
The audit-trail / dead-letter record for every run, stored in `schedule_executions`. One row per claimed slot or manual fire.
