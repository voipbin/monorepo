...
func (r *requestHandler) AIV1AIcallParticipantList(ctx context.Context, aicallID uuid.UUID, size uint64, token string) ([]*amparticipant.WebhookMessage, error) {
	uri := fmt.Sprintf("/v1/aicalls/%s/participants?page_size=%d&page_token=%s", aicallID, size, url.QueryEscape(token))
...
// it returns a list of participant webhook messages if it succeeds.
func (r *requestHandler) AIV1AIParticipantList(ctx context.Context, aiID uuid.UUID, size uint64, token string) ([]*amparticipant.WebhookMessage, error) {
	uri := fmt.Sprintf("/v1/ais/%s/participants?page_size=%d&page_token=%s", aiID, size, url.QueryEscape(token))
