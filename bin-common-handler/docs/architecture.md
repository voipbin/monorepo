...
- `publishDelayedEvent` (internal, unexported) can publish through the `bin-manager.delay` exchange for delayed delivery, but is unreachable from any of the 5 exported `NotifyHandler` methods today — every real caller of the internal `publishEvent` hardcodes `delay=0`. VOIP-1407 design §7 item 3 left this dead code path as a deliberate, deferred, optional follow-up rather than removing or wiring it up.
