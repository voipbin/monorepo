...
// subscribeTargets set wired in cmd/number-manager (the flow-manager and
// customer-manager event exchanges). The fanout QueueSubscribe calls in Run()
// stay until VOIP-1407 as the rollback/degrade surface.
...
func (h *subscribeHandler) Run() error {
	...
	go func() {
		if errConsume := h.sockHandler.ConsumeMessage(context.Background(), h.subscribeQueue, "number-manager", false, false, false, 10, h.processEventRun); errConsume != nil {
