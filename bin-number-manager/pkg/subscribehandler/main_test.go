...
func Test_Run_sequencing(t *testing.T) {
	...
	queueName := string(commonoutline.QueueNameNumberSubscribe)
	...
	mockSock.EXPECT().ConsumeMessage(gomock.Any(), queueName, "number-manager", false, false, false, 10, gomock.Any()).Return(nil).AnyTimes()
