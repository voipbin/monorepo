...
// Test_topicPatterns_golden pins the EXACT bind set of number-manager on the global topic
...
func Test_topicPatterns_golden(t *testing.T) {
	expected := []string{
		...
		"flow-manager.flow.*.deleted",
	}

	// design §5: number-manager binds exactly 2 patterns.
...
// Test_fanoutUnbindTargets_golden pins the fanout exchanges number-manager unbinds after
// a fully successful topic bind (VOIP-1406). This is the FULL subscribeTargets set wired
// in cmd/number-manager: the flow-manager and customer-manager event exchanges.
