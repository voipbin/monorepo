...
func run(sqlDB *sql.DB, cache cachehandler.CacheHandler, cfg *config.Config) error {
	...
	notifyHandler := notifyhandler.NewNotifyHandler(sockHandler, requestHandler, commonoutline.QueueNameConferenceEvent, serviceName, os.Getenv("CLICKHOUSE_ADDRESS"))
