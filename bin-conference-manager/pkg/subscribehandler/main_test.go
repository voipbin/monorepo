...
func Test_Run_sequencing(t *testing.T) {
	...
	queueName := string(commonoutline.QueueNameConferenceSubscribe)
	...
	mockSock.EXPECT().ConsumeMessage(gomock.Any(), queueName, "conference-manager", false, false, false, 10, gomock.Any()).Return(nil).AnyTimes()
