...
// ServiceAgentConversationGets sends a request to chat-manager
// to getting the given agent's list of conversation.
// it returns list of conversation if it succeed.
func (h *serviceHandler) ServiceAgentConversationGets(ctx context.Context, a *amagent.Agent, size uint64, token string) ([]*cmconversation.WebhookMessage, error) {
	// log := logrus.WithFields(logrus.Fields{
	// 	"func":        "ServiceAgentConversationGets",
	...
	// res, err := h.conversationGets(ctx, size, token, filters)
	// if err != nil {
