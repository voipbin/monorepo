...
insight          Restricted to the Insight tool set (``get_contact_interactions``, ``get_conversation_content``, ``get_related_cases``, ``get_case_notes``, ``get_contact_profile``) and a dedicated system prompt. Intended for agent-facing Q&A over a contact-manager Case (e.g., an agent asking the AI questions about a Case while working it in square-talk), not for handling live calls.
================ =======================================
