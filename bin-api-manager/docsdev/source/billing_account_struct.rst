...
* ``cost_type`` (enum string): Classification of the billing cost. Values include: ``call_pstn_outgoing``, ``call_pstn_incoming``, ``call_vn``, ``call_extension``, ``sms``, ``number``, ``number_renew``.
