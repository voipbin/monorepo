...
* ``direct_hash`` (String): Hash for direct extension access. Empty string when direct access is disabled. When enabled, this hash forms the direct SIP URI: ``sip:direct.<hash>@sip.voipbin.net``. Enable via ``PUT /extensions/{id}`` with ``"direct": true``. Regenerate via ``POST /extensions/{id}/direct-hash-regenerate``.
