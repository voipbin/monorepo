...
* ``domain_name`` (String): The full SIP domain (realm) of the extension, in the format ``{label}.reg.voipbin.net`` where ``{label}`` is a short customer-specific identifier (e.g. ``ab12``). Auto-generated by VoIPBIN. Use this value verbatim when configuring SIP devices. Accounts created before the domain migration may still see the legacy format ``{customer-id}.registrar.voipbin.net``, which remains valid until the migration completes.
