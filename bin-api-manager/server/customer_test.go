...
func Test_customerPut_NameOnly(t *testing.T) {
	// Regression test for the PUT /customer partial-update fix
	...
	req, _ := http.NewRequest(http.MethodPut, "/customer", bytes.NewBuffer(reqBody))
	...
	mockSvc.EXPECT().CustomerSelfUpdate(
		req.Context(), agent,
		&expectName,
		(*string)(nil),
		(*string)(nil),
