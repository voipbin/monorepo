...
// Test_campaignsPost_MissingAuthIdentity verifies PostCampaigns emits the
// canonical UNAUTHENTICATED / AUTHENTICATION_REQUIRED envelope when
// auth_identity is missing from the gin context.
func Test_campaignsPost_MissingAuthIdentity(t *testing.T) {
	assertMissingAuthIdentity(t, http.MethodPost, "/campaigns",
		[]byte(`{"name":"n","detail":"d","type":"call","service_level":1,"end_handle":"stop","actions":[],"outplan_id":"00000000-0000-0000-0000-000000000000","outdial_id":"00000000-0000-0000-0000-000000000000","queue_id":"00000000-0000-0000-0000-000000000000","next_campaign_id":"00000000-0000-0000-0000-000000000000"}`))
}

// Test_campaignsPost_InvalidJSONBody verifies PostCampaigns rejects malformed
// JSON with INVALID_ARGUMENT / INVALID_JSON_BODY.
func Test_campaignsPost_InvalidJSONBody(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, "/campaigns", bytes.NewBufferString("{not json"))
	req.Header.Set("Content-Type", "application/json")
	r.ServeHTTP(w, req)
...
// Test_campaignsIDPut_InvalidID verifies that a malformed UUID in the path
// triggers INVALID_ARGUMENT / INVALID_ID before the servicehandler is
// consulted.
func Test_campaignsIDPut_InvalidID(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	req, _ := http.NewRequest(http.MethodPut, "/campaigns/not-a-uuid", bytes.NewBufferString(`{}`))
	req.Header.Set("Content-Type", "application/json")
	r.ServeHTTP(w, req)
...
// Test_campaignsIDStatusPut_InvalidJSONBody verifies PutCampaignsIdStatus
// rejects malformed JSON with INVALID_ARGUMENT / INVALID_JSON_BODY before
// the servicehandler is consulted.
func Test_campaignsIDStatusPut_InvalidJSONBody(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	req, _ := http.NewRequest(http.MethodPut, "/campaigns/a8ba6662-540a-11ec-9a9f-b31de1a77615/status",
