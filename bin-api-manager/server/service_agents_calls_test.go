...
func Test_callsGET(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			reqQuery: "/service_agents/calls?page_token=2020-09-20T03:23:20.995000Z&page_size=10",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockSvc.EXPECT().ServiceAgentCallList(req.Context(), tt.agent, tt.expectPageSize, tt.expectPageToken).Return(tt.responseCalls, nil)
...
// Test_serviceAgentsCallsIDGet_InvalidID verifies
// GetServiceAgentsCallsId rejects a malformed UUID in the path with
// INVALID_ARGUMENT / INVALID_ID before the servicehandler is consulted.
func Test_serviceAgentsCallsIDGet_InvalidID(t *testing.T) {
