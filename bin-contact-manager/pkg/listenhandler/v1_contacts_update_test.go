...
func TestProcessV1ContactsAddressesIDPut(t *testing.T) {
	tests := []struct {
		name            string
	...
	}{
		{
			name: "update address",
			request: &sock.Request{
				URI:      "/v1/contacts/c31676f0-4e69-11ec-afe3-77ba49fae527/addresses/d1111111-1111-1111-1111-111111111111",
				Method:   sock.RequestMethodPut,
				DataType: "application/json",
				Data:     []byte(`{"target":"+1-555-999-8888","is_primary":true}`),
			},
			contactID: uuid.FromStringOrNil("c31676f0-4e69-11ec-afe3-77ba49fae527"),
			addressID: uuid.FromStringOrNil("d1111111-1111-1111-1111-111111111111"),
			responseContact: &contact.Contact{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockContact.EXPECT().UpdateAddress(gomock.Any(), tt.contactID, tt.addressID, gomock.Any()).Return(tt.responseContact, nil)
...
func TestProcessV1ContactsAddressesIDPut_ShortURI(t *testing.T) {
	mc := gomock.NewController(t)
	defer mc.Finish()
	...
	// Too short URI - missing address ID
	request := &sock.Request{
		URI:      "/v1/contacts/c31676f0-4e69-11ec-afe3-77ba49fae527/addresses",
		Method:   sock.RequestMethodPut,
		DataType: "application/json",
		Data:     []byte(`{"target":"+1-555-999-8888"}`),
	}
...
func TestProcessV1ContactsAddressesIDPut_InvalidJSON(t *testing.T) {
	mc := gomock.NewController(t)
	defer mc.Finish()
...
func TestProcessV1ContactsAddressesIDPut_UpdateError(t *testing.T) {
	mc := gomock.NewController(t)
	defer mc.Finish()
	...
	mockContact.EXPECT().UpdateAddress(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Return(nil, fmt.Errorf("database error"))
