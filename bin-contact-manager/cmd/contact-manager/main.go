...
import (
	...
	"monorepo/bin-contact-manager/pkg/subscribehandler"
)
...
var rootCmd = &cobra.Command{
	Use:   "contact-manager",
	Short: "Contact Manager Service",
	Long:  `Contact Manager is a microservice that manages CRM-style contact records with phone numbers, emails, and tags.`,
	RunE:  run,
}
...
func runService(sqlDB *sql.DB, cache cachehandler.CacheHandler) error {
	...
	if err := runSubscribe(sockHandler, contactHandler); err != nil {
		return err
	}
...
func runSubscribe(sockHandler sockhandler.SockHandler, contactHandler contacthandler.ContactHandler) error {

	subHandler := subscribehandler.NewSubscribeHandler(sockHandler, string(commonoutline.QueueNameContactSubscribe), contactHandler)
