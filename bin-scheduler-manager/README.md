...
- **MySQL** — `scheduler_schedules`, `scheduler_executions` (soft-delete via `tm_delete`); also the backup target for mysqldump
- **Redis** — per-schedule claim locks (redsync)
- **RabbitMQ** — listen queue `bin-manager.scheduler-manager.request`; subscribes to `bin-manager.customer-manager.event`; publishes internal events on `bin-manager.scheduler-manager.event`

## Local Development

```bash
# Build
cd bin-scheduler-manager
go build -o ./bin/ ./cmd/...
