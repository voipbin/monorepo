	...
	tests := []struct {
	...
	}{
		{
			...
			request: &sock.Request{
				...
				// pinned client shape: requesthandler.SchedulerV1ScheduleCreate
...
func Test_processV1SchedulesIDGet_notFound(t *testing.T) {
	...
	mockSchedule.EXPECT().Get(gomock.Any(), scheduleID).Return(nil, cerrors.NotFound(commonoutline.ServiceNameSchedulerManager, "SCHEDULE_NOT_FOUND", "The schedule was not found."))
	...
	tests := []struct {
	...
	}{
		{
			...
			request: &sock.Request{
				...
				DataType: "application/json",
				// pinned client shape: requesthandler.SchedulerV1ScheduleUpdate
...
func Test_processV1SchedulesIDExecutePost_error(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			responseErr: cerrors.FailedPrecondition(commonoutline.ServiceNameSchedulerManager, "EXECUTION_IN_PROGRESS", "The schedule already has a running execution."),
		...
		},
		{
			...
			responseErr: cerrors.NotFound(commonoutline.ServiceNameSchedulerManager, "SCHEDULE_NOT_FOUND", "The schedule was not found."),
