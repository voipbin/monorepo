package listenhandler

import (
	stderrors "errors"
	"fmt"
	"net/http"
	"testing"

	cerrors "monorepo/bin-common-handler/models/errors"
	commonoutline "monorepo/bin-common-handler/models/outline"

	pkgerrors "github.com/pkg/errors"

	"monorepo/bin-scheduler-manager/pkg/dbhandler"
)

func Test_errorResponse_typed(t *testing.T) {
	resp := errorResponse(cerrors.NotFound(commonoutline.ServiceNameSchedulerManager, "SCHEDULE_NOT_FOUND", "x"))
	if resp.StatusCode != http.StatusNotFound || resp.DataType != cerrors.DataTypeVoipbinError {
		t.Errorf("typed: got %d/%s", resp.StatusCode, resp.DataType)
	}
}

func Test_errorResponse_typedFailedPrecondition(t *testing.T) {
	resp := errorResponse(cerrors.FailedPrecondition(commonoutline.ServiceNameSchedulerManager, "EXECUTION_IN_PROGRESS", "x"))
	if resp.StatusCode != http.StatusConflict || resp.DataType != cerrors.DataTypeVoipbinError {
		t.Errorf("failed precondition: got %d/%s", resp.StatusCode, resp.DataType)
	}
}

func Test_errorResponse_typedAlreadyExists(t *testing.T) {
	if errorResponse(cerrors.AlreadyExists(commonoutline.ServiceNameSchedulerManager, "SCHEDULE_NAME_DUPLICATED", "x")).StatusCode != http.StatusConflict {
		t.Errorf("already exists failed")
	}
}

func Test_errorResponse_typedThroughPkgErrorsWrap(t *testing.T) {
	if errorResponse(pkgerrors.Wrap(cerrors.NotFound(commonoutline.ServiceNameSchedulerManager, "x", "x"), "outer")).StatusCode != http.StatusNotFound {
		t.Errorf("wrapped failed")
	}
}

func Test_errorResponse_legacyNotFound(t *testing.T) {
	if errorResponse(pkgerrors.Wrap(dbhandler.ErrNotFound, "x")).StatusCode != http.StatusNotFound {
		t.Errorf("legacy failed")
	}
}

func Test_errorResponse_unclassifiedError(t *testing.T) {
	if errorResponse(fmt.Errorf("x")).StatusCode != http.StatusInternalServerError {
		t.Errorf("unclassified failed")
	}
}

func Test_errorResponse_nilError(t *testing.T) {
	if errorResponse(nil).StatusCode != http.StatusInternalServerError {
		t.Errorf("nil failed")
	}
}

func Test_errorResponse_typedNotFoundFromBusinessHandler(t *testing.T) {
	typed := cerrors.NotFound(commonoutline.ServiceNameSchedulerManager, "SCHEDULE_NOT_FOUND", "x").Wrap(dbhandler.ErrNotFound)
	if errorResponse(typed).StatusCode != http.StatusNotFound {
		t.Errorf("e2e failed")
	}
	if !stderrors.Is(typed, dbhandler.ErrNotFound) {
		t.Errorf("Is should walk")
	}
}
