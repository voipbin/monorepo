# bin-scheduler-manager Architecture

## Component Overview

`bin-scheduler-manager` is the platform-internal cron scheduler. Schedules live in MySQL; every replica runs the same dispatch tick loop and competes for due slots via Redis locks plus a DB compare-and-swap claim, then fires the schedule's RPC target over RabbitMQ and records an execution audit row.

```mermaid
graph TD
    CMD[cmd/scheduler-manager] --> LH[pkg/listenhandler]
    CMD --> SH[pkg/subscribehandler]
    CMD --> DH[pkg/dispatchhandler]
    CTL[cmd/scheduler-control] --> DB[pkg/dbhandler]
...
|---------|------|-----------|
| `cmd/scheduler-manager` | Daemon entry point; wires handlers, starts listen/subscribe/dispatch loops | cobra `rootCmd` |
| `cmd/scheduler-control` | Admin CLI over direct DB/cache (no RabbitMQ) — emergency disable path | cobra commands |
| `internal/config` | Flag/env binding (Viper), global config singleton | `config.Config` |
| `pkg/listenhandler` | Consumes `bin-manager.scheduler-manager.request`; regex-routes to handlers; error → `cerrors.VoipbinError` envelope | `ListenHandler` |
...
| `pkg/backuphandler` | `mysqldump --single-transaction` subprocess, gzip to `SCHEDULER_BACKUP_DIR`, retention pruning | `BackupHandler` |
| `pkg/dbhandler` | MySQL via squirrel; CAS claim transaction; sqlite-backed tests | `DBHandler` |
| `pkg/cachehandler` | Redis + redsync try-once claim locks (`scheduler:lock:<id>`) | `CacheHandler` |
...
Requests arrive on the RabbitMQ queue `bin-manager.scheduler-manager.request`. The `listenhandler` dispatches by matching the URI against compiled regexes (`pkg/listenhandler/main.go`).
...
CRUD and execution outcomes publish internal events via notifyhandler on `bin-manager.scheduler-manager.event`:
