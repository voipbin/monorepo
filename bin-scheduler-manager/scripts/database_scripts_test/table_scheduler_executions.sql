...
create index idx_scheduler_executions_schedule on scheduler_executions(schedule_id, tm_create);
create index idx_scheduler_executions_reap on scheduler_executions(status, tm_deadline);
create index idx_scheduler_executions_tm_create on scheduler_executions(tm_create);

-- hard DB-level backstop against double-fire (design §4.2)
create unique index uq_scheduler_executions_schedule_scheduled on scheduler_executions(schedule_id, trigger_type, tm_scheduled);
