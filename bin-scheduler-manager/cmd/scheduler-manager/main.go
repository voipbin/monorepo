...
import (
	...
	"monorepo/bin-scheduler-manager/pkg/listenhandler"
	"monorepo/bin-scheduler-manager/pkg/schedulehandler"
	"monorepo/bin-scheduler-manager/pkg/subscribehandler"
)

const serviceName = commonoutline.ServiceNameSchedulerManager
...
func main() {
	rootCmd := &cobra.Command{
		...
		Short: "Voipbin Scheduler Manager Daemon",
...
func runDaemon() error {
	...
	log.WithField("config", config.Get()).Info("Starting scheduler-manager...")
	...
	<-chDone
	cancel()
	log.Info("The scheduler-manager stopped safely.")
	return nil
}
...
func runServices(ctx context.Context, sqlDB *sql.DB, cache cachehandler.CacheHandler) error {
	...
	notifyHandler := notifyhandler.NewNotifyHandler(sockHandler, reqHandler, commonoutline.QueueNameSchedulerEvent, serviceName)

	scheduleHandler := schedulehandler.NewScheduleHandler(reqHandler, db, notifyHandler)
	backupHandler := backuphandler.NewBackupHandler(cfg.DatabaseDSN, cfg.SchedulerBackupDir, cfg.SchedulerBackupRetentionCount)
	dispatchHandler := dispatchhandler.NewDispatchHandler(db, cache, reqHandler, notifyHandler, cfg.SchedulerTickIntervalSec, cfg.SchedulerDispatchConcurrency)

	if err := runListen(sockHandler, scheduleHandler, dispatchHandler, backupHandler, cfg.SchedulerExecutionRetentionDays); err != nil {
		return err
	}
...
func runListen(
	...
	// run
	if err := listenHandler.Run(string(commonoutline.QueueNameSchedulerRequest), string(commonoutline.QueueNameDelay)); err != nil {
...
func runSubscribe(sockHandler sockhandler.SockHandler, scheduleHandler schedulehandler.ScheduleHandler) error {
	...
	subHandler := subscribehandler.NewSubscribeHandler(
		sockHandler,
		string(commonoutline.QueueNameSchedulerSubscribe),
		subscribeTargets,
		scheduleHandler,
	)
