...
func NewAudioHandler(ctx context.Context, awsAccessKey string, awsSecretKey string, gcpEndpoint string) AudioHandler {
	log := logrus.WithField("func", "NewAudioHandler")

	gcpClient, err := gcpGetClient(ctx, gcpEndpoint)
	if err != nil {
