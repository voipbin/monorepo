...
- `bin-api-manager` (agent CRUD, login, status management)
- `bin-flow-manager` (agent action dispatch from call flows)
- `bin-queue-manager` (agent availability queries for routing)
- `bin-talk-manager` (agent lookup for talk sessions)
...
- `bin-manager.webhook-manager.event`
