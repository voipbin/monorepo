...
func run(sqlDB *sql.DB, cache cachehandler.CacheHandler) error {
	...
	recordingHandler := recordinghandler.NewRecordingHandler(reqHandler, notifyHandler, db, bridgeHandler)
	confbridgeHandler := confbridgehandler.NewConfbridgeHandler(reqHandler, notifyHandler, db, cache, bridgeHandler, recordingHandler, externalMediaHandler)
