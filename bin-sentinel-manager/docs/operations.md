...
| `sentinel_manager_container_event_stream_reconnect_total` | Counter | `result` (`delivered`\|`empty`) | Docker event stream attempts that ended. A healthy stream blocks rather than returning, so a rising `empty` rate means the socket proxy is unreachable — sentinel is up but watching nothing. `maxConsecutiveEmptyStreams` (20, ≈1 min) consecutive `empty` results exit the process |
