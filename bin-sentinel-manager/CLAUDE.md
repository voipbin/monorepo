...
A `pod-reader` Role granting `get`/`list`/`watch` on pods in the `voip` namespace must be bound to the service account **before** deployment. **This service does not ship or maintain example Kubernetes manifests** — a self-hoster running the Kubernetes backend configures their own `Deployment`/`Role`/`RoleBinding` for their own cluster, same as they would for any third-party workload. Without that RBAC in place, the informer's initial `List` is denied and the process exits at startup once the sync deadline expires.
...
- Deployment (docker): `komodo/docker-compose.yml`, single replica, deployed by CircleCI's `bin-sentinel-manager-deploy` job. Sets `SENTINEL_BACKEND=docker` as a hardcoded literal. This is the only deployment descriptor this repository owns — it is what bm-nyc-01 actually runs
- Deployment (kubernetes): **no manifest is shipped from this repository.** The Kubernetes backend is application capability, not a deployment we operate — a self-hoster running Kubernetes writes their own `Deployment`/`Role`/`RoleBinding` for their own cluster and sets `SENTINEL_BACKEND=kubernetes` on it themselves. (An earlier revision of this repo carried a `k8s/*.yml` reference deployment; it was removed as out of scope for this repository to own — see git history if a starting point is useful, but it is not maintained here and may drift from what the current code expects.)
- Every deployment descriptor MUST carry a `SENTINEL_BACKEND` value regardless of who owns it. Startup validation has no default, so an omitted value crash-loops that deployment
