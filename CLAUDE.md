...
This is the VoIPbin monorepo - a unified backend codebase for a cloud-native CPaaS platform. It contains 38 services (35 `bin-*` + 3 `voip-*`-proxy) providing VoIP, messaging, conferencing, AI integration, and communication workflow orchestration.
...
**Why:** `bin-common-handler` is a globally shared library — every change triggers verification across all 38 services. Keeping it lean reduces blast radius and maintenance burden.
...
- ✅ **Service / business handlers** (`pkg/*handler` other than `listenhandler`: `eventhandler`, `callhandler`, `aicallhandler`, `toolhandler`, etc.) MUST take and return **domain values** — for inputs, the unwrapped domain parameters (`ctx`, ids, filters, pagination args, or a domain `models/` type), never a wire `request.*` DTO; for outputs, domain `models/` types, never a wire `response.*` DTO.
- ✅ **Handler inputs are passed as unwrapped parameters, not a wrapped request struct.** Follow the established signature style: `List(ctx, size, token, filters)` / `Get(ctx, id)`. Do not thread a transport (or transport-shaped) request struct through the handler; the `listenhandler` unmarshals the wire `request.*` DTO and passes the individual domain arguments. (Returning a small domain result struct for multi-field outputs, e.g. a paginated `EventListResponse{Result, NextPageToken}`, is fine — this rule is about inputs.)
- ✅ **`listenhandler` is the single layer that touches transport DTOs.** On the way in it unmarshals the wire `request.*` DTO and calls the handler with the unwrapped domain arguments. On the way out, pick one of two sanctioned serialization styles per resource:
  - **(A) Direct domain marshal — preferred for a single resource or a simple collection of one domain type.** The domain `models/` type carries json tags and *is* the wire shape; `listenhandler` takes the handler's domain result and `json.Marshal`s it directly, with no `response.*` DTO. A paginated result struct (e.g. `EventListResponse{Result, NextPageToken}`) is still style (A) as long as it is just the domain type marshaled directly. References: `bin-flow-manager` `v1FlowsIDGet` marshals `*flow.Flow` directly; `bin-timeline-manager` `v1CorrelationsGet` marshals `*correlation.Correlation` directly; `bin-call-manager` `processV1CallsGet` marshals a `[]*call.Call` domain result directly.
  - **(B) Response-DTO mapping — use only when the wire shape must genuinely differ from any one domain type.** Have `listenhandler` map the handler's domain result(s) into a `response.*` DTO immediately before `json.Marshal`. Choose this when the response composes fields from several domain values, reshapes them, wraps a bare primitive/scalar that is not a domain type, or exposes a deliberately narrowed projection — i.e. there is no single domain type whose shape already equals the wire shape. Reference: `bin-billing-manager` `processV1AccountsIDIsValidBalancePost` calls `accountHandler.IsValidBalance(...) (bool, error)` and wraps the bare `bool` into `response.V1ResponseAccountsIDIsValidBalance{Valid bool}` — a wire-only shape that corresponds to no domain model. Do NOT introduce a `response.*` DTO that is a field-for-field, identical-json-tag copy of a domain type — that is style (A) wearing a DTO costume and is exactly the drift-prone boilerplate this section warns against.
...
- ❌ **NEVER** accept a `request.*` value (or pass a transport-shaped request struct) into, or return a `response.*` value from, a handler interface method. (Under style (A) the handler still returns its domain `models/` type — that type simply carries json tags so it can be marshaled directly. Putting json tags on a domain model is allowed and is not the same as returning a `response.*` DTO.)

**Reference pattern:** `bin-call-manager` — `callHandler.List(ctx, size, token, filters)` returns `[]*call.Call` (a domain type) and `listenhandler.processV1CallsGet` unmarshals the wire request, calls with unwrapped args, and marshals the domain result directly (style (A)). `bin-timeline-manager` — `eventHandler.List(ctx, publisher, resourceID, events, pageToken, pageSize)` returns `*event.EventListResponse` (domain); `listenhandler.v1EventsPost` unmarshals `request.V1DataEventsPost`, calls `List` with the unwrapped fields, then marshals the result. (Today `v1EventsPost` copies it field-for-field into an identical `response.V1DataEventsPost`; since that DTO is a 1:1 same-tag copy of the domain `EventListResponse`, it is effectively style (A) and the copy is redundant — a new resource in this shape should just marshal the domain type directly.)
...
**Why:** Keeps the transport contract owned by one layer. Handler *logic* stays free of wire-format concerns — json tags on a domain model are declarative metadata, not logic, so style (A) does not drag wire handling into the business code. Request/response shapes can still evolve, and each wire DTO has exactly one place to audit. Style (A) is the default for a single resource or simple collection because a separate DTO would be duplicated boilerplate that can silently drift from the domain; reach for style (B) only when the wire shape genuinely cannot be any single domain type (composition, reshape, or a narrowed subset).
