...
- ✅ **`listenhandler` is the single layer that touches transport DTOs.** On the way in it unmarshals the wire `request.*` DTO and calls the handler with the unwrapped domain arguments. On the way out it maps the handler's domain result into a `response.*` DTO immediately before `json.Marshal`.
...
- ❌ **NEVER** accept a `request.*` value (or pass a transport-shaped request struct) into, or return a `response.*` value from, a handler interface method.

**Reference pattern:** `bin-call-manager` — `callHandler.List(ctx, size, token, filters)` returns `[]*call.Call` (a domain type) and `listenhandler.processV1CallsGet` unmarshals the wire request, calls with unwrapped args, and marshals the domain result. `bin-timeline-manager` — `eventHandler.List(ctx, publisher, resourceID, events, pageToken, pageSize)` returns `*event.EventListResponse` (domain); `listenhandler.v1EventsPost` unmarshals `request.V1DataEventsPost`, calls `List` with the unwrapped fields, and maps the domain result into `response.V1DataEventsPost`.
