...
func Test_Run_sequencing(t *testing.T) {
	...
	queueName := string(commonoutline.QueueNameDirectSubscribe)
	...
	mockSock.EXPECT().ConsumeMessage(gomock.Any(), queueName, string(commonoutline.ServiceNameDirectManager), false, false, false, 10, gomock.Any()).Return(nil).AnyTimes()
