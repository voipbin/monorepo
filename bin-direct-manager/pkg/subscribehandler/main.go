...
// set wired in cmd/direct-manager. The fanout QueueSubscribe calls in Run() stay until
...
func (h *subscribeHandler) Run() error {
	...
	go func() {
		if errConsume := h.sockHandler.ConsumeMessage(context.Background(), h.subscribeQueue, string(commonoutline.ServiceNameDirectManager), false, false, false, 10, h.processEventRun); errConsume != nil {
