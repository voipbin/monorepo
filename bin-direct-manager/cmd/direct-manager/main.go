...
import (
	...
	"monorepo/bin-direct-manager/pkg/listenhandler"
	"monorepo/bin-direct-manager/pkg/subscribehandler"
)
...
var rootCmd = &cobra.Command{
	Use:   "direct-manager",
	Short: "Direct Manager Service",
	Long:  `Direct Manager is a microservice that manages direct hash lookups for resources.`,
	RunE:  run,
}
...
func runService(sqlDB *sql.DB, cache cachehandler.CacheHandler) error {
	...
	if err := runSubscribe(sockHandler, directHandler); err != nil {
		return err
	}
...
func runListen(sockHandler sockhandler.SockHandler, dHandler directhandler.DirectHandler) error {
	listenHandler := listenhandler.NewListenHandler(sockHandler, dHandler)

	// run
	if err := listenHandler.Run(string(commonoutline.QueueNameDirectRequest), string(commonoutline.QueueNameDelay)); err != nil {
...
func runSubscribe(sockHandler sockhandler.SockHandler, dHandler directhandler.DirectHandler) error {

	subscribeTargets := []string{
		string(commonoutline.QueueNameCustomerEvent),
	}
	subHandler := subscribehandler.NewSubscribeHandler(sockHandler, string(commonoutline.QueueNameDirectSubscribe), subscribeTargets, dHandler)
