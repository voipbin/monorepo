...
|---------|--------------|------------|
| Queuecalls stuck in `waiting` with available agents | Agent tag mismatch (queue tags ≠ agent tags); agent-manager events not reaching queue-manager; both event entry points (§3.5) missed their trigger | Verify queue `tag_ids` matches at least one agent's tags; check subscribehandler logs for `agent_status_updated` events; the matching backstop (§5.2) retries every waiting queuecall every 30s, so a stuck queuecall should self-heal within that window -- if it does not, check for a stuck Redis reconcile lease (`redis-cli GET queue:reconcile:match`) |
| Queuecall stays in `connecting` forever | Agent did not answer (conference join failed); call-manager confbridge error | Check conference-manager logs; verify agent address is reachable; the matching backstop's connecting-stale recovery (§5.2) hangs up and rolls back any queuecall stuck in `connecting` past ~150s automatically -- if it is not recovering, check the backstop's Redis lease and `reconcileConnectingStale` error logs |
| Wait timeout not firing | The delayed `timeout_wait` RabbitMQ message never arrived (broker down/message lost at `Create` time); queuecall already progressed to `service` | Check RabbitMQ delayed-message exchange health; check queuecall status directly; manually call `POST /v1/queuecalls/{id}/timeout_wait` if the caller should be abandoned now |
