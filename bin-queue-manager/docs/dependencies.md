# Dependencies: bin-queue-manager

> Auto-generated from `docs/reference/extractor.sh`. Do not edit manually.
> Regenerate: `bash docs/reference/extractor.sh bin-queue-manager && bash docs/reference/render-deps.sh bin-queue-manager`

## Inbound Callers

Services that send RPC requests to this service's queue:

- `bin-api-manager` (queue CRUD, queuecall management)
- `bin-flow-manager` (queue action dispatch from call flows)
- `bin-call-manager` (health-check and kick via service type routing)
...
Webhook events this service publishes: none (queue-manager does not publish domain events; state changes are communicated via RPC responses and agent/call event subscriptions).
