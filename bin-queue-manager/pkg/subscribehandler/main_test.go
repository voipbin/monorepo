...
func Test_Run_sequencing(t *testing.T) {
	...
	queueName := string(commonoutline.QueueNameQueueSubscribe)
	...
	mockSock.EXPECT().ConsumeMessage(gomock.Any(), queueName, "queue-manager", false, false, false, 10, gomock.Any()).Return(nil).AnyTimes()
