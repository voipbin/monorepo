...
// every topicPatterns bind has succeeded (VOIP-1406). It equals the full
// subscribeTargets set wired in cmd/queue-manager: the agent and conference
// subscriptions are dead binds (zero dispatch cases) and are dropped together with the
// live call-manager fanout leg (design §4). The fanout QueueSubscribe calls in Run()
