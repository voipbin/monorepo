...
- `bin-api-manager` (flow CRUD, activeflow management)
- `bin-call-manager` (creating and stopping activeflows during call lifecycle)
...
Webhook events this service publishes: none (flow-manager does not publish domain events; it acts as an orchestrator that invokes other services).
