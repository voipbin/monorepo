...
func (h *activeflowHandler) updateNextAction(ctx context.Context, activeflowID uuid.UUID, caID uuid.UUID) (*activeflow.Activeflow, error) {
	...
	if af.ForwardStackID != stack.IDEmpty && af.ForwardActionID != action.IDEmpty {
		...
		resStackID, resAct, err = h.stackHandler.GetAction(af.StackMap, af.ForwardStackID, af.ForwardActionID, true)
		if err != nil {
		...
		resStackID, resAct = h.stackHandler.GetNextAction(af.StackMap, af.CurrentStackID, &af.CurrentAction, true)
	}
