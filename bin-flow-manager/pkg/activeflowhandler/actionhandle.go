...
func (h *activeflowHandler) actionHandleConnect(ctx context.Context, af *activeflow.Activeflow) error {
	...
	resCalls, resGroupcalls, err := h.reqHandler.CallV1CallsCreate(ctx, f.CustomerID, f.ID, af.ReferenceID, &opt.Source, opt.Destinations, earlyExecution, executeNext, opt.Anonymous)
	if err != nil {
...
func (h *activeflowHandler) actionHandleCall(ctx context.Context, af *activeflow.Activeflow) error {
	...
	resCalls, resGroupcalls, err := h.reqHandler.CallV1CallsCreate(ctx, af.CustomerID, flowID, masterCallID, opt.Source, opt.Destinations, opt.EarlyExecution, false, opt.Anonymous)
	if err != nil {
		log.Errorf("Could not create a outgoing call for connect. err: %v", err)
		return err
	}
...
func (h *activeflowHandler) actionHandleAITalk(ctx context.Context, af *activeflow.Activeflow) error {
	...
	sv, err := h.reqHandler.AIV1ServiceTypeAIcallStart(ctx, assistanceType, assistanceID, af.ID, referenceType, af.ReferenceID, 30000)
	if err != nil {
		return errors.Wrap(err, "Could not start the service.")
	}
