...
2. If found: set `Metadata.ContactCaseID` on the returned Conversation via the new `ConversationUpdateMetadata`-style RPC (whole-struct replace semantics, matching `bin-customer-manager`'s `UpdateMetadata` shape).
