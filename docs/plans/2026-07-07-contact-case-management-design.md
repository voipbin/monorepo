...
1. `ConversationGetBySelfAndPeer(self=source, peer=destination)` — get-only, per §4.4's round-7 correction; matches the real function signature (`bin-conversation-manager/pkg/dbhandler/conversation.go:155`, keyed by `(self, peer)` only, no `customer_id` parameter — see §4.4's own tenant-isolation note on why this is safe). If the Conversation doesn't exist yet, the subsequent send (step 3 below) creates it the normal way via the existing `GetOrCreateBySelfAndPeer` inside the send path, and this endpoint's metadata-write step is simply skipped for this call (first-ever message to this destination has no prior Conversation to annotate — same accepted narrow degradation §4.4 already documents for the inbound direction, now symmetric for outbound).
