...
- Case tagging via a new `contact_case_tag_assignments` junction table in `bin-contact-manager` (mirroring the existing Contact-specific `contact_tag_assignments` pattern — see §7 for why `bin-tag-manager` itself has no generic "taggable resource" mechanism to register against).
...
| Per-`reference_type` keying (or an explicit precedence rule) for `Conversation.Metadata.ContactCaseID` when more than one non-`conversation_message` `reference_type` writes to it | At launch only `call` writes this field, so the single-scalar last-writer-wins limitation flagged in §4.4 is not an active defect | Before a second non-`conversation_message` `reference_type` (e.g. a hypothetical future video/chat channel) adopts the §4.4 write pattern |
...
- **Written proactively, before any message exists on the peer:** when contact-manager opens a non-`conversation_message` Case (today: `call`), it looks up (get-only, never creates) the sibling message `Conversation` for that peer and sets `Metadata.ContactCaseID` on it. This is unchanged from §4.4's mechanism.
...
**Write path (this section's direction) — up to two RPCs, with an explicit failure-handling rule.** When contact-manager's Case get-or-create (§4) opens a *new* Case (not on cache-hit reuse of an already-linked one — see cost note below) for a `reference_type` other than `conversation_message` (today: a `call` case), it makes **up to two sequential RPCs** to conversation-manager:
...
**Read path.** `MessageEventReceived` already loads the `Conversation` row for every inbound SMS to evaluate `getExecuteMode`; per §4.3, `MessageEventSent` does the same for outbound. That existing load is extended to also read `conversation.Metadata.ContactCaseID` and include it as the single `case_id` hint field on the `conversation_message_created` event payload (published via `message.EventTypeMessageCreated` and consumed on the contact-manager side by `contactHandler.EventConversationMessageCreated`, `bin-contact-manager/pkg/contacthandler/interaction.go:118` — corrected symbol names; there is no `EventTypeConversationMessageCreated` constant in the codebase) contact-manager subscribes to, regardless of direction. No additional DB read, no additional RPC — the value rides on data conversation-manager already has in hand for an unrelated purpose.
...
2. **Destination-to-case binding (the check this section exists to add).** `destination` must be attributable to this specific case, checked in this order:
   - If `case.contact_id` is set (the case has a matched Contact): `destination` must appear in that Contact's registered addresses (`bin-contact-manager`'s existing `AddressListByContactID` — corrected symbol name; `pkg/dbhandler/address.go:320`, reused as-is — no new address-matching logic). This is the strict check: an agent cannot use this endpoint to message a number unrelated to the Contact this case represents, even if they know the case_id.
   ...
   - Otherwise: reject with a single generic 4xx message (e.g. "destination is not associated with this case") that does **not** distinguish which of the two binding branches failed — this avoids giving a caller who already holds a valid `case_id` in their own tenant a differential oracle to infer whether a probed number belongs to the matched Contact versus not matching `peer_target` either. This is the concrete safety mechanism that prevents case_id from being usable as a bare capability token to message an arbitrary number — a caller cannot launder an unrelated send through someone else's case just by knowing its ID.
   - **Known race window (not blocking, documented per this design's own race-disclosure convention):** this check is a plain read, not wrapped in the same transaction as the metadata write or send below — unlike §4's careful `FOR UPDATE` treatment of the Case row. If `case.contact_id` or the Contact's address list changes between this check and the send, the send could go through against a destination that was valid only microseconds earlier. Impact is bounded: the destination is fixed by the caller's own request, so this is an ordinary check-then-act staleness window with a benign failure mode, not an attacker-controlled bypass — nothing here lets a caller land on a destination they couldn't have gotten validated at some point. Not solved here; flagged so it isn't mistaken for an oversight.
3. **Source ownership validation (round-17 correction — a real, previously-unstated gap, not merely a documentation error).** An earlier draft of this section claimed source-ownership was "already enforced" by the existing outbound-send path. That claim is false: `ConversationMessageSend` (`bin-api-manager/pkg/servicehandler/conversation_message.go:107-144`) takes an already-resolved `conversationID`, not a `source` address, and its only check is `hasPermission` against the conversation's existing `CustomerID` — it never validates that `source` belongs to the calling customer. In today's system this gap is latent, not exploitable, because `Conversation.Self` is always populated from an inbound/outbound `message-manager` webhook event derived from a number validated upstream at provisioning time, never from direct agent input. §4.5 is the **first** agent-facing path where `source` comes straight from request JSON, and feeding it unchecked into the new `ConversationGetOrCreateBySelfAndPeer` RPC (§4.5 step 1) would create/reuse a Conversation with an unverified `Self` — allowing an agent to send a message that appears to originate from a number their customer doesn't actually own, with no gate analogous to the one call-manager already enforces for outbound calls (`bin-call-manager/pkg/callhandler/outgoing_call.go:780-874`, `getValidatedSourceForOutgoingCall`, which queries `NumberV1NumberList` with `FieldCustomerID`+`FieldNumber`+`FieldType=Normal`+`FieldStatus=Active` before allowing a call to proceed with a given source).

   **Required, not optional:** before step 1 below, this endpoint's handler must perform the equivalent check — query `NumberV1NumberList` (`bin-number-manager`, via `bin-common-handler`'s existing requesthandler) with `FieldCustomerID=<this case's customer_id>`, `FieldNumber=source`, `FieldType=Normal`, `FieldStatus=Active`, `FieldDeleted=false`, and reject the request with a 4xx if no match is found. This is new validation code this endpoint must add — explicitly scoped as required implementation work, not something to inherit for free from an existing path (unlike destination-binding and rate-limiting, which genuinely are inherited via the unchanged `ConversationMessageSend` call in step 3).

**Execution.** Unlike §4.4's case-linking write (which is deliberately get-only because it fires from a Case-open event where no message has ever been sent — see §4.4's round-7 correction on why creating a Conversation there would trigger a spurious `conversation_created` webhook), §4.5 is the opposite situation: an agent is about to genuinely send a real message to `destination`, so if no Conversation exists yet for `(self=source, peer=destination)`, creating one here is correct and truthful — a real message really is about to cross that peer, which is exactly the condition `GetOrCreateBySelfAndPeer` (`bin-conversation-manager/pkg/conversationhandler/db.go:43-86`) exists to gate on. Concretely, and only after the source-ownership check above passes:

1. **Resolve `conversationID`** via a new RPC exposing the existing internal `GetOrCreateBySelfAndPeer(self=source, peer=destination)` cross-service — this is a distinct RPC from §4.4's get-only `ConversationGetBySelfAndPeer`, and get-or-create semantics are the correct choice specifically here, not a relaxation of §4.4's round-7 correction. (Round-12 correction: an earlier draft of this section assumed `ConversationMessageSend`'s existing send path — `bin-api-manager/pkg/servicehandler/conversation_message.go:107` → `bin-conversation-manager/pkg/conversationhandler/message.go:20`'s `MessageSend`, which calls `h.Get(ctx, conversationID)`, not `GetOrCreateBySelfAndPeer` — would create-on-miss internally. It does not: `MessageSend` only accepts an already-resolved `conversationID` and looks it up by ID, never by `(self, peer)`. Resolving `conversationID` must happen explicitly, as this new RPC call, before calling the existing send path.)
2. Set `Metadata.ContactCaseID = case_id` on the resolved Conversation via `ConversationUpdateMetadata` (§4.4's existing RPC, not a new one) — applies whether the Conversation was just found or just created. **Failure handling: fail open on this step, never skip the send.** If this RPC fails, log it and proceed to step 3 anyway — the message still gets sent; only the case-linking optimization for this send is lost (the resulting `MessageEventSent` simply carries no hint, exactly as if §4.3/§4.4 didn't exist for this one message). This mirrors §4.4's own failure-handling rule for its two-RPC sequence, applied here to the same class of risk.
3. Send the message via the existing `ConversationMessageSend(conversationID, text, medias)` (unchanged), now passing the `conversationID` resolved in step 1.

**Round-13 correction — the step-1-succeeds-but-step-3-never-runs window.** If step 1 creates a brand-new Conversation (firing a genuine `conversation_created` webhook, per `GetOrCreateBySelfAndPeer`'s existing behavior) but the request is then aborted, the caller's connection drops, or the handler crashes before step 3 executes — as opposed to step 3 executing and failing, which still leaves a real `Message` row per the provider-agnostic `Send`/`sendSMS`/`sendLine` handlers in `bin-conversation-manager/pkg/messagehandler/send.go` — the result is a real Conversation with a fired `conversation_created` webhook and zero messages ever landing on it. This is structurally the same class of harm §4.4's round-7 correction was written to prevent, now reachable through this endpoint specifically because step 1 must create eagerly (per the "a real message is genuinely about to be sent" rationale above) rather than deferring creation until the send itself succeeds. **Accepted as a named, narrow degradation, not solved here:** unlike §4.4's proactive Case-open write (which never has a real message pending and can safely skip creation entirely on a miss), §4.5 cannot avoid creating eagerly without abandoning the case-linking hint for genuinely-new-destination sends altogether — and an abandoned request between step 1 and step 3 (client abort, crash, timeout) is a rare failure mode, not the common path. A webhook consumer seeing `conversation_created` with no subsequent message is a real, if rare, false signal; this is flagged here rather than silently left unaddressed, matching this document's own convention of naming accepted degradations rather than hiding them.
...
| Tags | **Round-22 correction.** An earlier draft claimed `Case` could be "registered as a taggable resource type" in `bin-tag-manager` with "no new tag storage needed." That claim is false: `bin-tag-manager` (confirmed against `models/tag/tag.go` and its full RPC surface `TagV1TagCreate/Update/Delete/Get/List` in `bin-common-handler/pkg/requesthandler/tag_tag.go`) manages only the tag *label* itself — there is no `resource_type`/`resource_id` concept, no generic assignment table, and no "register a resource type" API anywhere in that service. The actual tag-*assignment* mechanism lives in `bin-contact-manager`, hardcoded to `Contact`: `pkg/dbhandler/tag_assignment.go`'s `TagAssignmentCreate/Delete/ListByContactID` operate on a `contact_tag_assignments` junction table (`contact_id`, `tag_id`, `tm_create`) that is not generic or reusable as-is. The corrected design: add a new, analogous `contact_case_tag_assignments` junction table (`case_id`, `tag_id`, `tm_create`) in `bin-contact-manager`, plus `CaseTagAssignmentCreate/Delete/ListByCaseID` dbhandler methods and corresponding `contacthandler`/RPC surface mirroring the existing Contact pattern exactly. `bin-tag-manager` itself needs zero changes — Cases reference the same `Tag` rows (by `tag_id`) that Contacts already do; only the assignment (linking) table is new, and it belongs to contact-manager, not tag-manager. | The tag *label* CRUD service already exists and is correctly reused as-is; only the Case-to-tag linking mechanism is new work, scoped honestly as a schema addition rather than assumed free |
...
5. Create `contact_case_tag_assignments` table in `bin-contact-manager` (`case_id`, `tag_id`, `tm_create`, `bin-contact-manager`-owned — round-22 correction, §7: `bin-tag-manager` has no generic resource-registration mechanism; this mirrors the existing `contact_tag_assignments` junction table pattern exactly, referencing `bin-tag-manager`'s existing `Tag` rows by ID).
...
- **bin-contact-manager**: `CaseHandler` (get-or-create with the bounded retry loop per §4.2, close with truthful-persisted-state response per §5.1, continue reusing the same get-or-create primitive per §5.3, assign/unassign, list, list-unresolved), `CaseNoteHandler` (create/list/delete, emits `case_note_created` via a plain `PublishEvent` — never `PublishWebhookEvent` — per §3.5), Resolution handler extended with the case-level derivation hook and case-scoped list/delete operations (§3.3), `case-control` CLI (including `reconcile-contact`), and new `CaseTagAssignmentCreate/Delete/ListByCaseID` dbhandler methods + RPC surface against the new `contact_case_tag_assignments` table (§7 round-22 correction — mirrors the existing Contact-specific `tag_assignment.go` pattern, `bin-tag-manager` itself unchanged). New RPC call to `bin-conversation-manager` when a non-`conversation_message` Case opens, to set `Conversation.Metadata.ContactCaseID` on the sibling message Conversation for the same peer (§4.4).
- **bin-conversation-manager**: new `Metadata` field on `Conversation` (nullable JSON column, mirroring `bin-customer-manager`'s `Metadata` pattern); a **new get-only RPC** (`ConversationGetBySelfAndPeer` — route + listenhandler + `bin-common-handler` client method, exposing the existing internal `GetBySelfAndPeer` lookup) — deliberately get-only, not get-or-create, so a miss creates nothing and fires no webhook (§4.4 round-7 correction); a **separate new get-or-create RPC** (`ConversationGetOrCreateBySelfAndPeer` — exposing the existing internal `GetOrCreateBySelfAndPeer`, cross-service for the first time) for §4.5's agent-send path specifically, where creating a Conversation on miss is correct because a real message is genuinely about to be sent (round-12 correction — distinct from, and not a relaxation of, the get-only RPC above); a new `ConversationUpdateMetadata`-shaped RPC for contact-manager to call; and `MessageEventReceived`/`MessageEventSent` extended to echo `Metadata.ContactCaseID` onto the `conversation_message_created` payload already published (`message.EventTypeMessageCreated`, §4.3/§4.4). No change to `getExecuteMode` or any flow/agent-routing dispatch logic — the new field is inert with respect to conversation-manager's own decisions.
- **bin-tag-manager**: no changes — `Tag` rows are reused as-is by ID; only `bin-contact-manager` gains new schema/handlers (see the `bin-contact-manager` bullet above and §7's round-22 correction).
- **bin-api-manager**: `/v1.0/cases`, `/v1.0/cases/{id}`, `/v1.0/cases/{id}/close`, `/v1.0/cases/{id}/continue`, `/v1.0/cases/{id}/notes`, **`POST /v1.0/cases/{id}/messages`** (§4.5 — send a case-linked message with explicit `source`/`destination`, validated against the case's matched Contact or bare `peer_target`, **plus a new source-ownership check against `NumberV1NumberList` (round-17 correction — not inherited from any existing path)**, then routed through the existing `ConversationMessageSend` path with a `Conversation.Metadata.ContactCaseID` write beforehand); extend `/v1.0/interactions` responses with `case_id`. `GET /v1.0/cases` supports filter query params `status`, `owner_type`/`owner_id`, and an unresolved filter (`contact_id IS NULL` combined with `status=open`) — mapping directly to the `idx_case_owner`/`idx_case_unresolved` indexes (§3.1) and the `list`/`list-unresolved` handler capabilities (§9's `bin-contact-manager` bullet above) that already exist to serve exactly these two day-one operator views ("my open cases," "unresolved queue"). Inherits the existing `PermissionProjectSuperAdmin` cross-customer bypass automatically (§7) — no new authorization code path.
...
- `POST /v1.0/cases/{id}/messages` (§4.5): rejects a send on a closed case (must `/continue` first); **rejects `source` numbers not owned by the case's customer as an active normal number (round-17: explicit test against the new `NumberV1NumberList` check — not a real number, wrong customer's number, inactive, or non-normal type all rejected)**; destination-to-case binding accepts a `destination` in the matched Contact's address list, accepts a bare `peer_target` match when the case is unresolved, and rejects any other destination with a single generic 4xx (explicit test that the rejection message does not distinguish which binding branch failed, and that case_id cannot be used as a bare capability token to message an unrelated number); the `Conversation.Metadata.ContactCaseID` write happens before the send and is correctly reflected on the resulting `MessageEventSent`'s hint field, verified end-to-end against §4.3's unified consumption path; first-ever-destination case (no prior Conversation) resolves a new `conversationID` via `ConversationGetOrCreateBySelfAndPeer`, and the metadata write still runs against that newly-created Conversation (not skipped) before the send; a `ConversationUpdateMetadata` RPC failure fails open — the send still proceeds, only the hint is lost — verified this does not block or fail the overall request.
