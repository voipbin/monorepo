...
3. **Source validation and rate-limiting/quota, both fully inherited from the existing send path — not reimplemented here.** `bin-api-manager`'s `ConversationMessageSend` (`pkg/servicehandler/conversation_message.go:107-144`) already enforces that the calling agent's `customer_id` (from `auth.AuthIdentity`, not from anything this endpoint's own case/destination validation touches) owns the `source` number before dispatch; because §4.5 step 3 below routes through this exact same call unchanged, that check applies identically here — an agent cannot spoof a different customer's `source` number through this endpoint any more than through the existing send API. Any existing per-customer/per-agent send quota or rate-limiting on that path is likewise inherited automatically, since this is not a new send path, merely a new entry point that resolves a `case_id` down to the existing `(source, destination, text)` call.

**Execution.** Unlike §4.4's case-linking write (which is deliberately get-only because it fires from a Case-open event where no message has ever been sent — see §4.4's round-7 correction on why creating a Conversation there would trigger a spurious `conversation_created` webhook), §4.5 is the opposite situation: an agent is about to genuinely send a real message to `destination`, so if no Conversation exists yet for `(self=source, peer=destination)`, creating one here is correct and truthful — a real message really is about to cross that peer, which is exactly the condition `GetOrCreateBySelfAndPeer` (`bin-conversation-manager/pkg/conversationhandler/db.go:43-86`) exists to gate on. Concretely:
