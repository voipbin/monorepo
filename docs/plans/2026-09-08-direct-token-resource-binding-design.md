...
- 작성일: 2026-09-08
- 개정: rev11 (계획 리뷰에서 발견된 사실 정정 반영. 리셋 규칙을 구현 계획과 동기화. 설계 결정은 변경 없음)
...
| 리셋 | **재부팅 이후 발행한 리소스 생성 요청이 2xx이고 계기 6을 발동시키지 않았을 때 0으로 초기화.** 그 외 어떤 요청도 리셋하지 않습니다. (초판은 "인증된 요청이 한 번 성공하면"이었으나, WebSocket 연결·재초기화의 정리 삭제·이력 조회가 각각 리셋해 상한을 무력화하는 것이 구현 계획 리뷰에서 발견돼 화이트리스트로 바꿨습니다. 구현 계획 §3.2 참조.) 이 리셋이 없으면 대화를 두 번 종료한 방문자(계기 1)가 영구히 죽습니다 |
...
- `HashFingerprint`: **JWT 서명 키로 키잉한 HMAC-SHA256 hex, 절단 없음.** `HMAC(jwtKey, 도메인 접두사 || 해시)` 형태입니다.

  **초판은 키 없는 SHA-256을 지정했고, 키잉을 반대하기까지 했습니다("솔트나 시간 의존 요소를 넣으면 replica 간에 성립하지 않는다"). 그 반대는 틀렸고, 구현은 키잉된 형태로 나갔습니다.**

  - 반대 논거가 성립하지 않는 이유: `jwtKey`는 **JWT를 서명하는 바로 그 키**입니다. replica 전체가 이미 공유하고 있고, 교체하면 토큰 서명 자체가 무효가 되므로 지문이 새로운 운영 결합을 만들지 않습니다. 솔트나 시각과 달리 프로세스마다 달라지지 않습니다.
  - 키잉이 필요한 이유: 원본 해시의 엔트로피가 **48비트**뿐이고(`generateHash`가 6바이트, `bin-direct-manager/pkg/directhandler/handler.go:24-30`) 형식이 알려져 있습니다. 그리고 토큰은 해시보다 멀리 갑니다. WebSocket 경로가 `*auth.AuthIdentity`를 통째로 로깅하므로, 키 없는 다이제스트는 **로그 한 줄에서 원본 해시를 역산**할 수 있게 하고 그 역산 결과는 토큰 만료와 `BootExpire` 상한을 모두 넘겨 삽니다.
  - 도메인 접두사는 같은 키를 쓰는 JWT 서명과 메시지 공간을 분리합니다. 없어도 악용은 어렵지만, 있으면 그 논증 자체가 필요 없어집니다.
  - 절단하지 않는 이유는 위의 48비트 때문입니다.

  **되돌리지 마십시오.** `Test_directHashFingerprint_isKeyed`와 `Test_directHashFingerprint_goldenVector`가 이 유도식을 고정하고 있으며, 값이 바뀌면 진행 중인 갱신이 전부 401이 됩니다.
...
| `ScopeVersion` | 스코프 계약의 판. 올리면 발급된 모든 direct 토큰이 한 번에 무효화됩니다 (9절) |
...
- 루프 방지 리셋: 재부팅 후 **생성 요청**이 2xx이면 카운터가 0이 되는지. WebSocket 연결·부팅·정리 삭제·이력 조회·메시지 전송은 리셋하지 않는지. **대화를 두 번 종료한 방문자(계기 1)가 계속 동작하는지.**
...
**이 절은 2026-09-09에 다시 썼습니다.** rev1~rev11의 3단계 착륙(발급 → 프론트 → 강제)은 대표님 결정으로 폐기됐고, 서버가 한 PR로 착륙했습니다. 폐기된 단계 구성과 그때 계산한 확률(1/8, 1/64, 2/9)은 **인용하지 마십시오.** 전부 "강제만 배포하며 재부팅 계약이 이미 깔려 있다"는, 지금은 성립하지 않는 전제 위의 값입니다.

**현재 유효한 착륙 분석은 구현 계획 `tasks/todo.md` §1.2.1입니다.** 병합 배포의 혼재 창(중간 시점 약 7/8, 매니저 선행 배포로 대략 절반)이 거기 있습니다.

아래 네 가지는 단계 구성이 사라져도 살아남는 사실이므로 여기에 남깁니다.

### 9.1 `ScopeVersion`이 왜 2에서 시작하는가

3단계 구성에서는 1단계가 `ScopeVersion: 1`을 발급하고 3단계가 2로 올려 그 집단을 일괄 무효화할 예정이었습니다. **병합 착륙에서는 1을 발급한 적이 없습니다.** 따라서 2는 그 폐기된 계획의 흔적입니다.

**이 값을 1로 "고치지" 마십시오.** 살아 있는 토큰이 전부 2를 들고 있어 `2 >= 1`이 통과하므로 그 변경은 **오늘은 조용히 무해합니다.** 대신 레버가 망가집니다. 다음에 실제로 무효화가 필요해 2로 올리면, 이미 2로 발급된 모든 토큰에 대해 아무 일도 일어나지 않습니다.

같은 경고를 상수 선언 옆(`bin-api-manager/pkg/servicehandler/main.go`)에도 두었습니다. 실제로 수정하려는 사람이 보는 줄은 그쪽입니다.

### 9.2 `buildJWTIdentity`의 두 거부 조건이 왜 둘 다 있는가

```
scope.AllowedResourceID == uuid.Nil        → 401
scope.ScopeVersion < DirectScopeVersionCurrent → 401
```

**오늘은 두 조건이 같은 집단을 가리킵니다.** 이 기능 이전에 발급된 토큰은 둘 다에 걸립니다. 그래서 버전 조건만 보면 죽은 코드처럼 읽힙니다.

죽은 코드가 아닙니다. 3단계 구성에서는 서로 다른 두 집단을 겨냥했고, 지금은 **앞으로 쓸 레버**로 남아 있습니다. 스코프 계약이 바뀌면(필드 의미 변경, 새 필수 필드 추가 등) 상수를 올리는 것만으로 발급된 모든 토큰이 한 번에 무효화되고, 클라이언트는 그 401을 재부팅 신호로 처리합니다. Nil 조건은 8.1이 다루며, 버전 조건의 근거는 여기입니다.

### 9.3 배포 순서 (유일하게 살아남는 운영 요구사항)

**매니저를 먼저 배포하고, 그다음 `bin-api-manager`를 배포합니다.**

1. `bin-ai-manager`와 `bin-webchat-manager`를 승인·배포하고 구버전 replica가 모두 사라졌는지 확인합니다.
2. 그다음 `bin-api-manager`를 승인합니다.

매니저 선행이 안전한 것은 증명 가능합니다. 두 매니저의 요청 DTO에 오늘 `id` 필드가 없으므로, 구버전 api-manager는 그 필드를 보내지 않고 신버전 매니저는 `uuid.Nil`로 읽어 기존 생성 경로를 탑니다. 즉 매니저만 먼저 나가는 구간에서는 아무 변화가 없습니다.

**순서를 어기면 전면 장애입니다.** api-manager가 먼저 나가면 지정 id를 보내도 구버전 매니저가 무시해 **모든 direct 대화가 불일치 상태로 생성**됩니다. 혼재 창의 소수 문제가 전체로 커집니다.

CI가 서비스별로 워크플로를 갈라 각자 승인 게이트를 가지므로 "같은 배포 단위"는 존재하지 않습니다. 동시성이 아니라 **순서**로 해결해야 합니다.

### 9.4 클라이언트 PR에 남는 제약

강제가 켜진 뒤 구버전 토큰은 401을 받습니다. 그 경로에 **재부팅 상한과 백오프가 반드시 있어야 합니다.** 없으면 부팅이 IP 한도에 걸려 429가 나오고, 방문자는 재연결이 아니라 정지를 봅니다.

재부팅 계기와 리셋 규칙의 전체 계약은 3.6에 있습니다. 그 절이 monorepo-javascript 쪽 PR의 명세입니다.
