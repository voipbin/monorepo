...
- 리셋이 없다고 가정하면 주기당 실패 확률 1/2, 소진 확률 **(1/2)³ = 1/8**. (넓은 리셋 규칙 전제의 값이며, 아래에서 무효화됩니다.)
- 리셋을 반영하면 주기당 실패는 **부팅이 구버전에 걸리고(1/2) 그 다음 요청이 신버전에 걸릴(1/2)** 때뿐이므로 1/4이고, 소진 확률은 **(1/4)³ ≈ 1/64**입니다. (같은 전제이며, 아래에서 무효화됩니다.)

rev6은 1/8만 적어 두 문단 뒤의 리셋 규칙과 모순됐습니다. 리셋이 모든 성공에 걸린다면 기대값은 1/64이고, 1/8은 리셋이 동작하지 않을 때의 보수적 상한입니다.

**구현 계획에서 리셋 규칙이 좁아졌으므로 실제 값은 아래와 같습니다.** 계획은 리셋 자격을 "재부팅 후 생성 요청이 2xx이고 계기 6을 발동시키지 않았을 때"로 한정했습니다. WebSocket 연결, 재초기화의 정리 삭제, 이력 조회가 각각 카운터를 리셋해 상한을 무력화하는 것이 발견됐기 때문입니다. 그 결과 소진 확률이 **약 22%(2/9)**가 됩니다.

주기당 결과가 셋이기 때문입니다. 종단 성공 1/4(신-신), 증가 1/2(신-구는 계기 6, 구-신은 401), 그리고 **리셋 후 재시도 1/4**(구-구. 생성은 성공해 리셋되지만 토큰이 구버전이라 다음 요청이 신버전에 닿으면 401). `f(c) = ½f(c+1) + ¼f(0)`, `f(3) = 1`을 풀면 `f(0) = 2/9`입니다.

결론(유계·fail-closed·새로고침으로 복구)은 그대로이나, **이 절의 1/64도 1/8도 인용하지 마십시오.** 둘 다 리셋이 모든 성공에 걸린다는 전제 위에 있습니다. 상한을 3에서 5로 올리면 약 6%(2/33)가 되며, 그 판단은 구현 계획 §3.2에 있습니다.

**리셋 조건이 여기서 두 번째로 중요해집니다.** **구버전 토큰(`scope_version < 2`)의 생성 요청**이 구버전 인스턴스에서 처리되면 성공하고, 그 성공이 카운터를 0으로 되돌립니다. (신버전 토큰의 생성이 구버전 인스턴스에 걸리는 경우는 200이지만 계기 6이 발동해 증가합니다.) 따라서 상한은 증가가 연속으로 쌓일 때만 걸리며, 정상 경로는 상한에 닿지 않습니다.

**단, 좁힌 리셋 규칙에서는 두 가지가 달라집니다.** 리셋 자격이 생성 요청에만 있으므로 "구버전에 걸린 아무 요청"이 리셋한다고 읽으면 안 되고, 증가 사유도 401 하나가 아니라 **계기 6 또는 401** 둘입니다. 즉 "연속 3회 401"이 아니라 "연속 3회 증가"입니다.
