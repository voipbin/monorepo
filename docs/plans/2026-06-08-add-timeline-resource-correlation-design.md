...
Status: Approved (v3, 3+ review rounds, design-change re-review passed)
...
- 백필을 포함한다 (Q5 확정). 과거 이벤트(000003 이전 INSERT 파트)의 `resource_id`/`activeflow_id` 빈 값을 채워 과거 통화도 correlation 대상에 포함한다.
- **운영 events 테이블 데이터 양이 적어 백필 비용이 작다 (Q6 확정). 시간대 제약 없이 언제든 적용 가능하다.** (대표님 확인, 2026-06-08) 테이블이 커지면 재평가.
- `MATERIALIZE COLUMN` / `MATERIALIZE INDEX`는 기존 파트를 백그라운드 머지로 재작성한다. 진행 상황은 `system.mutations`(`is_done`, `parts_to_do`)로 모니터링한다.
- migrate 성공 != 백필 완료(background mutation). 현재 데이터 양에서는 즉시 완료에 가깝지만, 원칙적으로 `system.mutations` 완료를 확인 후 feature를 enable한다 (2차 리뷰 #2).
- 롤백: `DROP INDEX` (즉시). `MATERIALIZE COLUMN`은 되돌릴 필요가 없다 (값 자체는 정상 추출이며 down에서 무동작).
- bloom_filter 인덱스가 `MATERIALIZED` 컬럼 위에서 동작함은 CH 24.3-alpine에서 확인됨 (Q7). 실효성(granule skip율)은 구현 시 `EXPLAIN indexes=1`로 실측.
...
`maxResources`: **100 (const var, config 미노출, Q8 확정).** 한 activeflow에 엮인 리소스가 100을 넘는 경우는 드물고(대형 컨퍼런스/캠페인 한정), 초과 시 `Truncated=true`로 알린다. 실제로 초과 케이스가 관측되면 상수만 상향한다. 정렬은 ClickHouse `ORDER BY first_seen`에서 1차, Go에서 publisher 그룹핑 후 그룹 내 재정렬로 안정화한다.

**Truncation 의미 (2차 리뷰 #5)**: LIMIT은 `first_seen ASC` 전역 정렬 후 적용되므로, 잘릴 때 전 publisher에 걸쳐 **가장 늦게 등장한** 리소스부터 누락된다. publisher별로는 일부 그룹이 불완전해 보일 수 있으나 결정적이다. 연속 토큰(continuation token)은 없다. maxResources를 초과하는 그래프는 cap 이상 조회 불가하며, 필요 시 `maxResources` 상수를 상향(코드 변경 + 재배포)해 대응한다.

## 7. RabbitMQ Integration

### Request URI

이 RPC는 부수효과 없는 리소스 조회이므로 REST 의미론상 **GET + path param**을 사용한다 (Q1 확정). 기존 events/aggregated_events가 POST인 것은 RPC 관례일 뿐 의미론적 근거는 GET 편이다.

```
GET /v1/correlations/<resource_id>
```

listenhandler 정규식 라우팅에 `/v1/correlations/([0-9a-fA-F-]+)` 형태로 등록한다 (대소문자 UUID 모두 허용, uuid.Parse가 최종 검증). 리소스 이름은 짧은 `correlations`로 한다 (단일 용도, 필요 시 후속 분기).

### 신규 파일

- `pkg/listenhandler/models/response/correlation.go` — 응답 DTO (4절 구조체)
- `pkg/listenhandler/v1_correlations.go` — listen 핸들러 (GET, path param)
- `pkg/listenhandler/main.go` — 라우팅 등록 (`/v1/correlations/([0-9a-fA-F-]+)`)
...
5. listenhandler `v1_correlations.go` + 라우팅 등록(`/v1/correlations/<id>`) + 테스트.
...
|---|---|---|---|
| Q1 | RPC 형태: GET path param vs POST body | **확정: GET `/v1/correlations/<resource_id>`** (부수효과 없는 리소스 조회) | CTO ✅ |
...
| Q5 | MATERIALIZED 백필 (000003 이전 과거 이벤트 공백) | **확정: 백필 포함.** migration 000004에 `MATERIALIZE COLUMN` 선행 단계 포함 | CTO ✅ |
| Q6 | skip index MATERIALIZE 적용 비용 (대용량 events) | **확정: 데이터 양 적어 비용 작음, 시간대 제약 없이 언제든 적용** (대표님 확인) | CTO ✅ |
| Q7 | bloom_filter 인덱스가 대상 CH 버전에서 MATERIALIZED 컬럼 위 정상 동작? | **확정: CH 24.3-alpine 확인** (install repo versions.yaml). MATERIALIZED는 물리 저장 컬럼이라 skip index 정상 적용. 단 실효성(granule skip율)은 구현 시 EXPLAIN 실측 | CTO ✅ |
| Q8 | maxResources 기본값 | **확정: 100 (const var, config 미노출)** | CPO/CTO ✅ |
...
## 15. Review Summary (3차 리뷰, 디자인 변경 후 재리뷰)

Q1(GET 전환)/Q5/Q6(백필 확정)/Q7(CH 24.3 확정)/Q8(maxResources=100 const) 변경 후 신규 독립 리뷰 라운드(CHANGES REQUESTED). blocking 1건 반영:

- **[Blocking] §6 truncation 노트의 "config 상향" 잔재가 Q8(const, config 미노출)와 모순** → "상수 상향(코드 변경 + 재배포)"으로 수정. (§6)
- **[Minor] 라우팅 정규식이 소문자 UUID만 허용** (uuid.Parse는 대문자 수용) → `[0-9a-fA-F-]+`로 확장, 대소문자 일관 처리. (§7, §11)
- naming(struct `V1DataResourceCorrelationGet` vs route `/correlations`)·GET 일탈·down에서 MATERIALIZE COLUMN 미해제는 모두 의도/적절로 승인(조치 불요).
