...
listenhandler 정규식 라우팅에 `/v1/correlations/([0-9a-f-]+)` 형태로 등록한다. 리소스 이름은 짧은 `correlations`로 한다 (단일 용도, 필요 시 후속 분기).
...
- `pkg/listenhandler/main.go` — 라우팅 등록 (`/v1/correlations/([0-9a-f-]+)`)
