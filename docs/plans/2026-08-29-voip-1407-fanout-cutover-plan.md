...
**For `bin-tts-manager` (R3 finding MEDIUM-1 -- resolved the plan's own open conditional and added the files it had left unstated):** design §4 and issue §3 both state `pkg/ttshandler` makes zero publish *calls* (nothing in its handler logic invokes a `NotifyHandler` method), but this does NOT mean the field is absent -- *verified this plan:* `pkg/ttshandler` DOES carry the field, so the deletion is NOT confined to `cmd/tts-control/main.go` as an earlier revision of this paragraph left open as an unresolved "re-verify at edit time" branch. Concretely:

- `bin-tts-manager/pkg/ttshandler/main.go:29` -- the `notifyHandler notifyhandler.NotifyHandler` struct field.
- `pkg/ttshandler/main.go:100-108` -- `NewTTSHandler(...)` takes `notifyHandler notifyhandler.NotifyHandler` as a parameter (`:107`), assigned into the struct at `:134`.
- `cmd/tts-control/main.go:38` (the construction) **and `:47`** (where that constructed value is passed into `NewTTSHandler`).
- **`cmd/tts-manager/main.go:128`** -- the **daemon** ALSO passes `notifyHandler` into `ttshandler.NewTTSHandler(...)`. This file was not otherwise in this plan's touched-file set for Unit C. Its own `NewNotifyHandler(...)` construction (`cmd/tts-manager/main.go:123`) is one of Unit A's 55 topic-only sites and is **untouched by Unit C** -- only the *downstream* `NewTTSHandler(...)` call that receives that already-correctly-constructed handler needs its argument dropped, matching the field/parameter deletion. (Keeping the field and passing `nil` from `tts-control` instead of deleting it would recreate exactly the nil-interface hazard design §2.3/§2.3.1 exists to eliminate -- so the daemon edit is mandatory, not optional.)
- `pkg/ttshandler/main_test.go:97` -- `notifyHandler: mockNotify` in a handler struct literal; drop the field and the now-unused `mockNotify` declaration/expectations in that test.

This makes tts-manager's actual diff shape symmetric with transfer-manager's (above): a field, a constructor parameter, both construction sites (control AND daemon), and at least one test file -- smaller than transfer's 8-file ripple, but not the single-file `cmd/tts-control/main.go`-only edit an earlier revision of this plan left as the untaken branch.
...
- `bin-call-manager/pkg/subscribehandler/binding_golden_test.go:38-66` (`Test_fanoutUnbindTargets_golden`, the only fanout-referencing test in this file besides `Test_topicPatterns_golden`, which is unaffected and untouched): pins `retained := "asterisk.all.event"` (`:59`) and asserts it never appears in the fanout-unbind set (`:64-65`). **Rewrite** to assert the retained-asterisk invariant against the new shape instead: the standalone `QueueSubscribe(QueueNameAsteriskEventAll)` statement exists in `Run()`, independent of any `subscribeTargets`/`fanoutUnbindTargets` var that no longer exists.
- `bin-timeline-manager/pkg/subscribehandler/binding_golden_test.go` -- **two** fanout-referencing tests (R2 finding LOW-4 corrected an earlier version of this bullet that named only one):
  - `Test_fanoutUnbindTargets_golden` (`:34-77`): pins the 25-entry fanout-unbind list and, at `:72-76`, that the asterisk leg never appears in it. **DELETE** -- once `fanoutUnbindTargets` is gone there is nothing left for this test to pin; there is no post-E1 equivalent to rewrite it into (the same disposition as design §3.3's "no more roll-back-on-partial-failure" ruling for the machinery this list served).
  - `Test_retainedFanoutTargets_golden` (`:79-112`): pins that `subscribeTargets` retains `asterisk.all.event` (`:86-93`), a `len(subscribeTargets) == 26` count (`:95`), a `subscribeTargets - fanoutUnbindTargets == 1` arithmetic invariant (`:98`), and the VOIP-1258 exchange name (`:104`). **Rewrite** to pin the same two retained invariants (asterisk leg present, VOIP-1258 block present) against the post-E1 code shape -- the count/arithmetic assertions have no post-E1 equivalent and are dropped, not replaced.
- `bin-agent-manager/pkg/subscribehandler/binding_golden_test.go` -- **two** fanout-referencing tests (R2 finding LOW-4):
  - `Test_fanoutUnbindTargets_Golden` (`:39-54`): the agent-manager analogue of call-manager's/timeline-manager's fanout-unbind-list pin. **DELETE**, same reasoning as timeline-manager's `Test_fanoutUnbindTargets_golden` above -- no post-E1 equivalent.
  - `Test_fanoutUnbindTargets_Retains1258TopicBind` (`:61-77`): has TWO halves, only one of which needs rewriting (R3 finding LOW-3 -- an earlier version of this bullet over-scoped the rewrite to both). The `fanoutUnbindTargets` loop (`:66-70`), asserting the VOIP-1258 topic exchange is never unbound, **must be rewritten** to assert the same invariant directly against the retained block's source, since `fanoutUnbindTargets` no longer exists to inspect. The `topicPatterns` loop (`:72-76`), asserting `"#"` never enters `topicPatterns`, needs **no change at all** -- `topicPatterns` survives the cutover verbatim as the sole intake mechanism (design §3.1), so this half of the test compiles and passes unmodified.
...
- `binding_golden_test.go` (the 17 **non-exception** services -- NOT the same grouping as E2's "Typical (18)"/webhook-manager split, R3 finding LOW-4: this 17 = the 20 minus the 3 exception services in E3.1, and DOES include webhook-manager, whose golden file is the plain generic shape, `bin-webhook-manager/pkg/subscribehandler/binding_golden_test.go:42-48`): drop the fanout-target pins. *Verified this plan:* all 20 services have this file.
...
**This table is `Run()`-focused and does NOT separately enumerate constructor tests (R1 finding MEDIUM-3; scope corrected and a second construction shape added by R2 findings MEDIUM-1/MEDIUM-2).** E1 item 4 / E2's "Typical (18)" row AND E2's `webhook-manager` row (its own constructor parameter, comma-joined string) both delete a `subscribeTargets`-family constructor parameter -- this breaks every test in **all 19 services that take one** (the 18 typical + webhook-manager; **timeline-manager is the sole exception**, since `NewSubscribeHandler(sockHandler, dbHandler)` there takes no targets parameter at all, `bin-timeline-manager/pkg/subscribehandler/main.go:119-128`, so its own `Test_NewSubscribeHandler` at `main_test.go:769` is genuinely unaffected).

Two distinct construction shapes break, not one:

1. **Direct `NewSubscribeHandler(...)` calls** -- drop the removed argument. Confirmed examples: `bin-tag-manager/pkg/subscribehandler/main_test.go:107` (`Test_NewSubscribeHandler`, local `subscribeTargets := []string{...}` at `:115`), `bin-contact-manager/pkg/subscribehandler/subscribehandler_test.go:203` (`TestNewSubscribeHandler`), `bin-transfer-manager/pkg/subscribehandler/main_test.go:15` (`TestNewSubscribeHandler`), and -- the R2-found webhook-manager case E4's `webhook` row does not otherwise name -- `bin-webhook-manager/pkg/subscribehandler/customermanager_test.go:252` (`TestNewSubscribeHandler`, constructing with the comma-joined literal `"target1,target2"` at `:260`; this file makes no `Run()`/`QueueSubscribe` call so it is invisible to the `Run()`-focused derivation OQ-3 describes -- do not miss it).
2. **`&subscribeHandler{... subscribeTargets: ...}` struct literals** -- drop the field from the literal. This is the shape behind three rows this table marks "keep"/"untouched" for an unrelated reason (their `QueueCreate`-only scope), which is still correct for their `Run()`-sequencing assertions but does NOT mean the file needs zero edits: `bin-tag-manager/pkg/subscribehandler/error_test.go:14` (`Test_Run_QueueCreateError`, literal at `:21-26` sets `subscribeTargets` at `:24`; also `:46` in the file's second test), `bin-contact-manager/pkg/subscribehandler/subscribehandler_test.go:216` (`Test_Run_QueueCreateError`, literal `:223-228` sets it at `:226`; also `:248,:271` elsewhere in the file), `bin-transfer-manager/pkg/subscribehandler/main_test.go:59` (`TestProcessEventRun`, literal `:66-72` sets `subscribeTargets: []string{}` at `:70`; also `:32,:38` elsewhere in the file). Repo scan for the same shape in call-manager: `main_test.go:31,42,58,105,115,133,178,240,295` -- all inside tests this table already schedules for full rewrite, so no separate action needed there.

Treat both shapes as a standing instruction for **all 19 non-timeline services with a targets parameter**: when deleting the constructor parameter/field, drop it from every call site AND every struct literal in that service's test files -- mechanical, not optional, and not separately itemized per-service below because it is uniform.
...
| contact | `run_sequencing_test.go`: `_success:30`, `_declareFailure:68`, `_bindFailure_noRollbackNeeded:101`, `_fanoutUnbindFailure_continues:135`; `subscribehandler_test.go`: `Test_Run_QueueCreateError:216`, `Test_Run_SubscribeError:238`, `Test_Run_Success:261`; `binding_golden_test.go` | `:30`/`:261` rewrite; `:68` confirm; `:101` rework; `:135` **DELETE**; `:216` keep its `QueueCreate`-only assertions, but drop the `subscribeTargets:` field from its handler literal at `:223-228` (R2 finding MEDIUM-2 -- also drop it from the other struct literals in this file at `:248,:271`); `:238` **DELETE** (fanout `QueueSubscribe` gone). |
...
| tag | `main_test.go`: `Test_Run_sequencing:135`; `error_test.go`: `Test_Run_QueueCreateError:14`, `Test_Run_QueueSubscribeError:36`; `binding_golden_test.go` | `:135` rewrite; `error_test.go:14` keep its `QueueCreate`-only assertions, but drop the `subscribeTargets:` field from its handler literal at `:21-26` (R2 finding MEDIUM-2 -- also `:46` for the file's second test); `error_test.go:36` **DELETE**. |
...
| transfer | `main_test.go`: `Test_Run_sequencing:110`, `TestProcessEventRun:59`; `binding_golden_test.go` | `:110` mechanical rewrite. `:59`'s `Run()`-sequencing assertions are untouched, but drop the `subscribeTargets: []string{}` field from its handler literal at `:66-72` (R2 finding MEDIUM-2 -- also `:32,:38` elsewhere in the file). **Note:** transfer-manager is ALSO in Unit C -- keep the two edits separate. |
| webhook | `main_test.go`: `Test_Run_TopicMigrationSequencing:23`; `customermanager_test.go`: `TestNewSubscribeHandler:252`; `binding_golden_test.go` | `:23` rewrite, accounting for the `strings.Split` deletion from E2. `customermanager_test.go:252` (R2 finding MEDIUM-1 -- not otherwise named, makes no `Run()`/`QueueSubscribe` call so is invisible to a `Run()`-focused sweep): drop the removed constructor argument from the call at `:260`. |
...
**F2a -- the 22 services with an actual code change in this PR (the 20 consumer services -- `transfer` and `webhook` already among them, not additional -- plus `talk-manager`/`tts-manager` from Units B/C; `bin-common-handler` itself is handled separately as F3, not counted here; R2 finding LOW-5 corrected an earlier "20+4" that double-counted transfer/webhook and left bin-common-handler ambiguous, and did not match §4.1's 23-module figure = these 22 + bin-common-handler):** docs are staged in the same commit as that service's code change, using the extractor:
...
- For F2a's 22 code-touching services, per-service docs are staged with that service's code commit, so they are covered by that service's already-run workflow. **This does NOT hold for F2b's 8 doc-only publisher daemons** (R1 finding HIGH-2) -- they have no code commit in this PR, so their doc update is its own commit, verified only by review (no build/test/lint applies).
...
- [ ] 17 non-exception services' `binding_golden_test.go` files updated (fanout pins dropped) -- this 17 includes webhook-manager, which is outside E2's "Typical (18)" grouping but still a plain generic golden file (R3 finding LOW-4).
- [ ] The 3 exception services' `binding_golden_test.go` files REWRITTEN (not blanket-dropped) per E3.1: call-manager's `Test_fanoutUnbindTargets_golden`, timeline-manager's `Test_retainedFanoutTargets_golden`, and agent-manager's `Test_fanoutUnbindTargets_Retains1258TopicBind` all still exist and pin the retained asterisk/VOIP-1258 invariants against the post-cutover code shape. Timeline-manager's and agent-manager's second fanout-list test (`Test_fanoutUnbindTargets_golden` / `Test_fanoutUnbindTargets_Golden` respectively) is correctly DELETED, not rewritten (no post-cutover equivalent).
...
**Single PR** per root `CLAUDE.md`'s "one PR per task" rule. This whole ticket is one logical change spanning `bin-common-handler` + 22 code-touched services + 8 doc-only services + docs (R3 finding LOW-2 corrected a stale "~24" here, left over from before R2's LOW-5 fix settled the actual count at §4.1's 23 modules = these 22 + bin-common-handler).
...
- bin-transfer-manager: Delete dead notifyHandler wiring from transfer-manager and
  transfer-control
- bin-tts-manager: Delete the dead notifyHandler field and constructor parameter from
  pkg/ttshandler, and both its construction sites (tts-control and tts-manager)
...
**OQ-1 -- RESOLVED (대표님 decision):** leave `bin-api-manager`'s residual machinery untouched in this PR; file a separate Jira cleanup follow-up. Matches this section's own recommendation below.
...
Design §4 calls the 3 dead-wiring deletions "trivial (remove an unused field, an unused constructor parameter, and the `NewNotifyHandler` call plus its now-unused imports)." *Verified this plan:* removing `bin-transfer-manager/pkg/transferhandler`'s `notifyHandler` field and constructor parameter mechanically touches **8 test files** in that package. `bin-tts-manager/pkg/ttshandler` carries the same shape at smaller scale -- one test file (`main_test.go:97`) plus, unlike transfer, a SECOND live construction site (the `tts-manager` daemon at `cmd/tts-manager/main.go:128`, alongside `tts-control`) -- fully specified in Unit C's own text now (R3 finding MEDIUM-1), not just here. The mechanical fix (drop the mock-notify argument from each construction) is unambiguous, but a second question is not: several of those tests may currently set `mockNotify.EXPECT()` expectations that would become unused variables. **Recommendation: proceed with the mechanical fix and delete any now-unused mock declarations** -- that is the only compiling outcome. Flagged because the design doc's "trivial" framing under-describes the diff size, and a reviewer comparing plan-to-design may read the larger diff as scope creep.

**OQ-3 -- RESOLVED (대표님 decision):** delete the now-meaningless tests as this plan's §3 E4 already specifies, do not rewrite them. Confirmed default; no change to E4's dispositions.
...
- [ ] All 9 commits (§2.2, including 5b and 8, added by round-1 fixes -- R2 finding MEDIUM-3 corrected a stale "7" here) on `VOIP-1407-Cutover-remove-fanout-dual-publish`.
