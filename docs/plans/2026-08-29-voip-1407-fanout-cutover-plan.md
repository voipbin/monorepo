...
- `bin-agent-manager/pkg/subscribehandler/binding_golden_test.go` -- **two** fanout-referencing tests (R2 finding LOW-4):
  ...
  - `Test_fanoutUnbindTargets_Retains1258TopicBind` (`:61-77`): pins that the VOIP-1258 topic exchange is never unbound and that `"#"` never enters `topicPatterns`. **Rewrite** to assert the same two invariants directly against the retained VOIP-1258 block's source, not via `fanoutUnbindTargets`/`topicPatterns` var inspection.
