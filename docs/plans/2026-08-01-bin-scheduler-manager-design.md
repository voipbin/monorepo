# bin-scheduler-manager — Design (VOIP-1283, Phase 1)
...
- New standard-convention monorepo service `bin-scheduler-manager` (MySQL via
...
bin-scheduler-manager/
  ...
  cmd/scheduler-manager/main.go        # daemon (cobra; Bootstrap/LoadGlobalConfig style)
  cmd/scheduler-control/main.go        # admin CLI (direct DB/cache, no RabbitMQ)
  ...
  scripts/database_scripts_test/{table_scheduler_schedules.sql,table_scheduler_executions.sql}
...
- `models/outline/queuename.go`: `QueueNameSchedulerEvent/Request/Subscribe`, plus an
...
- `models/outline/servicename.go`: `ServiceNameSchedulerManager = "scheduler-manager"`
- `pkg/requesthandler/scheduler_schedule.go` (+ `send_request.go` sender): typed client
  methods `SchedulerV1ScheduleCreate/Get/Gets/Update/Delete/Execute`,
  `SchedulerV1ExecutionGets` — consumers: VOIP-1280 doctor and api-manager (Phase 3).
  Zero in-repo callers at merge time is accepted (the doctor integration is the
  immediate next ticket); `scheduler-control` is NOT a consumer — it uses direct
...
- `.circleci/config.yml`: path-filter mapping line
  `bin-scheduler-manager/.*  run-bin-scheduler-manager true` (alphabetical position)
- `.circleci/config_work.yml`: `run-bin-scheduler-manager` pipeline parameter, workflow
...
(new `scheduler_` row), architecture dependency graph regeneration. No
...
Indexes: PK, `idx_scheduler_schedules_customer_name (customer_id, name)` (non-unique,
lookup only), `idx_scheduler_schedules_next_run (enabled, tm_next_run)`.
...
### 4.2 `scheduler_executions` (audit trail / dead-letter record)
...
Indexes: PK, `idx_scheduler_executions_schedule (schedule_id, tm_create)`,
`idx_scheduler_executions_reap (status, tm_deadline)` (serves the per-tick reap),
`idx_scheduler_executions_tm_create (tm_create)` (serves the age-based retention DELETE),
`uq_scheduler_executions_schedule_scheduled (schedule_id, trigger_type, tm_scheduled)` —
the unique key is a hard DB-level backstop against double-fire (second claimant's INSERT
fails).

Retention: rows older than `SCHEDULER_EXECUTION_RETENTION_DAYS` (default 90) are
...
```
every SCHEDULER_TICK_INTERVAL_SEC (default 10s):
...
1. **Redis lock (redsync, contention filter):**
   `mutex := locker.NewMutex("scheduler:lock:"+scheduleID, redsync.WithExpiry(30*time.Second), redsync.WithTries(1))`.
...
4. **Unique execution row (backstop):** INSERT `scheduler_executions(schedule_id,
...
`scheduler-manager` (set by `NewRequestHandler(sockHandler, serviceName)`). The
...
live run's deadline. Served by `idx_scheduler_executions_reap (status, tm_deadline)`.
...
### 5.7 Metrics (namespace `scheduler_manager`)
...
| `/v1/executions/prune` | POST | retention pruning (internal; invoked by the `execution-retention` schedule). Batched `DELETE WHERE id IN (SELECT id FROM ... WHERE tm_create < :cutoff LIMIT 1000)` in a loop — the subquery form (not `DELETE ... LIMIT`) is used because mattn/go-sqlite3 lacks `SQLITE_ENABLE_UPDATE_DELETE_LIMIT`, keeping the statement testable in the sqlite harness. Served by `idx_scheduler_executions_tm_create` |
...
(`scheduler:lock:<id>`) and the same §5.3 step 2 overlap guard apply (a manual run is
...
Backup is modeled as a normal `rpc` schedule targeting **scheduler-manager's own queue**,
...
`SCHEDULER_BACKUP_DIR/voipbin-<UTC timestamp>.sql.gz`, then prunes to
`SCHEDULER_BACKUP_RETENTION_COUNT` (default 7) newest files. Host/offsite copy stays an
...
- **Storage under `replicas: 2` (requirement, not deferred):** the backup job arrives on
  ...
  `SCHEDULER_BACKUP_DIR` MUST be one volume **shared by all scheduler replicas**
  ...
  **`SCHEDULER_BACKUP_DIR` has no default and must be explicitly configured** — a
...
- **Seeded disabled** (`enabled=0`, cron `0 2 * * *`): production GCP uses managed Cloud
  ...
  call or scheduler-control command, no redeploy).
...
| `database-backup` | `0 2 * * *` | scheduler-manager `/v1/backups` POST | `{}` | 1800000 | 0 |
| `execution-retention` | `30 2 * * *` | scheduler-manager `/v1/executions/prune` POST | `{}` | 600000 | 1 |
...
- **requesthandler client tests (bin-common-handler):** per rpc.md §9.6, the new
  `SchedulerV1*` methods get client-side tests asserting the marshaled wire shape
...
| SCHEDULER_TICK_INTERVAL_SEC | 10 | scan cadence |
| SCHEDULER_DISPATCH_CONCURRENCY | 10 | max in-flight jobs per replica |
| SCHEDULER_EXECUTION_RETENTION_DAYS | 90 | audit pruning |
| SCHEDULER_BACKUP_DIR | (no default — must be explicitly set; see §7) | |
| SCHEDULER_BACKUP_RETENTION_COUNT | 7 | |
...
- If the dispatch loop misbehaves, `scheduler-control schedule disable <name>` (direct
