...
> **F1 (HIGH — RESOLVED: per-customer concurrency cap implemented).** The 1-min cooldown only gates re-analysis of the SAME activeflow. It does NOT cap a customer triggering analysis across thousands of DISTINCT ended activeflows, each a fresh 1-3 LLM calls on the best model (plus a flow-manager RPC + a ClickHouse scan + a transcript list per call). On a customer-facing trigger this would be an uncapped paid-LLM faucet and a backend-DoS amplifier. **Resolved in P1:** a per-customer concurrency cap (`AnalysisCountProgressing` + `analysisMaxProgressingPerCustomer`, currently 20) gates genuinely-new analyses; triggering past the cap returns `ErrConcurrencyLimit` -> HTTP 429. The cap is enforced only on new analyses (not idempotent returns or reanalyze of an existing row), so it bounds concurrent in-flight spend per customer. A per-customer/day total quota remains a possible future addition given pchero's cost sensitivity.
...
- **H3/#5 concurrent Start race:** dup-key catch on the `UNIQUE(activeflow_id)` insert returns the in-flight row, no 500. (§5.2)
...
- **F1 (High) customer-facing fan-out cap — IMPLEMENTED.** The 1-min cooldown only gates re-analysis of the SAME activeflow; it does NOT cap analyzing thousands of DISTINCT activeflows. Resolved by a per-customer concurrency cap: `AnalysisCountProgressing` + `analysisMaxProgressingPerCustomer` (currently 20). Triggering a genuinely-new analysis past the cap returns `ErrConcurrencyLimit` -> HTTP 429. The cap is checked ONLY on new analyses (not on idempotent returns or reanalyze of an existing row). A per-customer/day quota remains a possible future addition. (§5.2, §7.1)
- **F8/F9 (Low):** REST<->RPC name remap documented; resurrection-guard/enum confirmed consistent. Under the hard-delete model the async write-back guards on `status='progressing'` alone (a row hard-deleted mid-analysis simply no longer exists, so the final UPDATE affects 0 rows) — security-relevant since customers can DELETE mid-analysis.
