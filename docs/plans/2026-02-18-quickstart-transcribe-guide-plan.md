# Quickstart Transcribe Guide Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a "Transcribe" section to the quickstart documentation that walks users through real-time speech-to-text with WebSocket events and webhook delivery.

**Architecture:** Single RST file (`quickstart_transcribe.rst`) included in the existing `quickstart.rst` via `.. include::` directive. Follows the same pattern as `quickstart_call.rst` and `quickstart_queue.rst`. Must follow AI-native RST writing guidelines from `bin-api-manager/CLAUDE.md`.

**Tech Stack:** Sphinx RST documentation

---

### Task 1: Create quickstart_transcribe.rst

**Files:**
- Create: `bin-api-manager/docsdev/source/quickstart_transcribe.rst`

**Step 1: Write the RST file**

Create `bin-api-manager/docsdev/source/quickstart_transcribe.rst` with the following exact content:
...
Run: `ls -la bin-api-manager/docsdev/source/quickstart_transcribe.rst`
Expected: File exists

---

### Task 2: Update quickstart.rst to include the new file

**Files:**
- Modify: `bin-api-manager/docsdev/source/quickstart.rst:18` (add include before line 20)

**Step 1: Add the include directive**

In `bin-api-manager/docsdev/source/quickstart.rst`, add `.. include:: quickstart_transcribe.rst` after line 18 (`.. include:: quickstart_queue.rst`) and before line 20 (`.. _quickstart_next:`).

The edited section should look like:
...
Run: `grep -n 'quickstart_transcribe' bin-api-manager/docsdev/source/quickstart.rst`
Expected: Shows the include line

---

### Task 3: Build HTML documentation and verify

**Files:**
- Rebuild: `bin-api-manager/docsdev/build/` (generated HTML)

**Step 1: Build the Sphinx docs**

Run:
```bash
cd ~/gitvoipbin/monorepo-worktrees/NOJIRA-Add-quickstart-transcribe-guide/bin-api-manager/docsdev && python3 -m sphinx -M html source build
```
Expected: Build succeeds with no errors. Warnings about missing images or cross-references to other docs are acceptable.

**Step 2: Verify the generated HTML contains the new section**

Run:
```bash
grep -l 'quickstart_transcribe' ~/gitvoipbin/monorepo-worktrees/NOJIRA-Add-quickstart-transcribe-guide/bin-api-manager/docsdev/build/html/quickstart.html
```
Expected: File found, meaning the transcribe section was rendered into the quickstart page.
...
```bash
cd ~/gitvoipbin/monorepo-worktrees/NOJIRA-Add-quickstart-transcribe-guide
git add bin-api-manager/docsdev/source/quickstart_transcribe.rst
git add bin-api-manager/docsdev/source/quickstart.rst
git add -f bin-api-manager/docsdev/build/
git add docs/plans/2026-02-18-quickstart-transcribe-guide-design.md
git add docs/plans/2026-02-18-quickstart-transcribe-guide-plan.md
```

Note: `git add -f` is required for the `build/` directory because the root `.gitignore` excludes `build/`.

**Step 2: Commit**

```bash
git commit -m "NOJIRA-Add-quickstart-transcribe-guide

Add real-time transcription quickstart guide to developer documentation.

- bin-api-manager: Add quickstart_transcribe.rst with end-to-end transcription walkthrough
- bin-api-manager: Include new section in quickstart.rst
...
Expected: Shows the new commit with the expected message.
...
```bash
git push -u origin NOJIRA-Add-quickstart-transcribe-guide
```

**Step 2: Create PR**

```bash
gh pr create --title "NOJIRA-Add-quickstart-transcribe-guide" --body "$(cat <<'EOF'
Add a real-time transcription section to the quickstart documentation. Walks users through
creating a transcription flow, provisioning a virtual number, subscribing to WebSocket events,
making a call, receiving transcript events, and verifying results.

- bin-api-manager: Add quickstart_transcribe.rst with 6-step transcription walkthrough
- bin-api-manager: Update quickstart.rst to include new transcribe section
- bin-api-manager: Rebuild HTML documentation
- docs: Add design and implementation plan documents
EOF
)"
