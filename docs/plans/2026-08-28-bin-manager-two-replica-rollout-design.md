...
| api | **done (VOIP-1442; deploy verification pending)** | Design-phase code trace found the previously-assumed blocker does not reproduce in the current code path: AudioSocket dial-back is direct-to-replica (not through Caddy round_robin), so the "request lands on the wrong replica" failure mode streamData was flagged for has no entry point today. AdvertiseIP() (PR #1209, deployed) is a prerequisite already satisfied. **Confirmed by a review-response-session direct-dial synthetic test** (§설계 결정 2 — an inbound extension-to-extension SIP call was placed via the existing sip-validator harness, landed in the call-in Stasis app, produced a genuine `progressing` call resource, and media_stream connected successfully for both `sln` and `audiosocket` encapsulation with zero "Could not set audiosock" across the currently-deployed single api-manager instance). Cross-replica confirmation (both of 2 replicas) is deferred to Stage 2 below, since only 1 replica is live until PR-A deploys. Standard scale pattern applied (no code change). |
