...
| pipecat | **done (P15+16; deploy verification pending)** | 본체+사이드카 페어를 4개 named service로 수동 복제, deploy.replicas 미사용. POD_IP는 각 서비스 고유 리터럴(pipecat-manager-1/-2), production 네트워크 alias로 기존 dns_sd 이름(pipecat-manager) 유지 — 코드 변경 없음. |
