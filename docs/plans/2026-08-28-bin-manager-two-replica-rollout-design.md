...
| rag | **done (P24; deploy verification pending)** | Already replica-safe: `DocumentClaimForProcessing` claims work via an atomic CAS UPDATE (same class of safety as schedule's claim machinery). Its own unbounded PostgreSQL pool (it bypasses `databasehandler.Connect()`) is covered by the same 2026-08-28 no-app-cap decision that resolved gating condition 1 — no pool-cap fix is planned. Scaled via the standard recipe (container_name removed, deploy.replicas:2) — no code changes needed |
