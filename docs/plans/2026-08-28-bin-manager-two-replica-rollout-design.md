...
**2026-09-10 Komodo-API sweep:** all 25 services still tagged "deploy
verification pending" below (P11's 21 + P12's flow/hook + P18's
transcribe + P19a's tts) were re-checked via `ListStacks` on
`https://komodo.voipbin.net` (no VPN/SSH needed — the API is
internet-reachable behind Cloudflare, same credential CI's
`komodo-api-deploy.sh` uses). Every one reports `status: running(2)`
with both `<service>-1`/`<service>-2` containers on the same image
digest (tts additionally shows its `tts-manager-http` sidecar at
`running(1)`, correct per design — sidecar stays single-instance). This
confirms the container-level half of the pending checklist (item 1 of
the workflow doc's post-deploy verification). Prometheus dns_sd target
count and `ReplicaDegraded` alert state (items 2-3) were not re-checked
in this sweep — Prometheus/Grafana are loopback-only on bm-nyc-01 and
need VPN/SSH, which this sweep deliberately did not open just for this
check (no incident signal, no reason to believe dns_sd disagrees with
Komodo's own view of the same containers). Rows below are updated to
**done (container-verified 2026-09-10)** to reflect this; if a genuine
Prometheus/alert check happens incidentally on other work, upgrade
these to the full "done" the tracker gives api/pipecat/etc.

| Service | State | Blocker / notes |
|---|---|---|
| schedule | **done** | PR #1212, live-verified 2026-08-27 |
| agent, ai, billing, call, campaign, conference, conversation, customer, direct, email, message, number, outdial, queue, registrar, storage, tag, talk, transfer, webchat, webhook | **done (container-verified 2026-09-10)** | Scaled in one batch (P11) after gating condition 1 was resolved by the 2026-08-28 no-app-cap decision. No per-pod queue, no naming dependency, no unguarded global state. No resource limits (deliberate: cAdvisor exposes no per-container series to size them from, and measured footprints are 16-69MiB against ~240GB free). Komodo `ListStacks` confirms `running(2)` for all 21 on 2026-09-10 — see sweep note above |
| flow | **done (container-verified 2026-09-10)** | Has a global mutex map, but it wraps redsync (Redis distributed locks) — replica-safe. Scaled together with hook in P12. Komodo confirms `running(2)` on 2026-09-10 |
| hook | **done (container-verified 2026-09-10)** | Caddy naming blocker resolved by P10: monorepo-etc `infra-caddy/config/Caddyfile` now resolves the Compose service name (`hook-manager`) via a `dynamic a` upstream instead of the fixed `voipbin-hook-manager` container name, so `container_name` removal no longer breaks routing. Scaled in P12. Note: 별도 network-alias 추가(로컬 계획의 P9)는 불필요 판정 — Compose 서비스명이 production 네트워크 DNS alias로 자동 등록됨을 P5/P7/P10에서 실증. Komodo confirms `running(2)` on 2026-09-10 |
| api | **done** | PR #1249/#178, live-verified 2026-09-02. Design-phase code trace found the previously-assumed blocker does not reproduce in the current code path: AudioSocket dial-back is direct-to-replica (not through Caddy round_robin), so the "request lands on the wrong replica" failure mode streamData was flagged for has no entry point today. AdvertiseIP() (PR #1209, deployed) is a prerequisite already satisfied. Confirmed pre-deploy by a review-response-session direct-dial synthetic test (§설계 결정 2 — an inbound extension-to-extension SIP call was placed via the existing sip-validator harness, landed in the call-in Stasis app, produced a genuine `progressing` call resource, and media_stream connected successfully for both `sln` and `audiosocket` encapsulation with zero "Could not set audiosock" across the single-instance pre-deploy state). Standard scale pattern applied (no code change). Deployed cleanly with no orphan container; 2/2 containers healthy. Docker embedded DNS confirmed correct (both replica IPs resolve under the `api-manager` network alias via `nslookup`). Prometheus dns_sd briefly discovered only 1 of 2 targets in the minutes right after deploy and `ReplicaDegraded` fired transiently during that window — self-corrected to `count(service="api-manager")==2` with zero dropped targets and the alert fully cleared within ~16 minutes, no code/config fix needed. Per-pod queue consumer count and a post-deploy cross-replica media_stream retest were not re-run (대표님 판단: container/DNS/alert evidence plus the pre-deploy synthetic test were judged sufficient). |
...
| transcribe | **done (container-verified 2026-09-10)** | Per-pod routing gap resolved by P17 (per-pod RPC routing redesign), unblocking the scale-out. POD_IP is a dead config field (parsed, never consumed) so no value change was needed; HostID stays a random UUID, not POD_IP. Komodo confirms `running(2)` on 2026-09-10 |
| tts | **done (container-verified 2026-09-10)** | POD_IP now resolves the sidecar's Compose service name (`tts-manager-http`), not its container name — no naming dependency remains. Sidecar stays single-instance. Komodo confirms main service `running(2)` + sidecar `running(1)` on 2026-09-10 |
...
| route | **done** | PR #1237/#175, live-verified 2026-09-01. Global provider healthcheck loop now guarded by a redsync cross-replica lock (`pkg/healthcheckhandler/health.go`) — only one replica probes providers per cycle. Lock fails open on Redis errors (blind-UPDATE `ProviderUpdateHealthStatus` makes concurrent unlocked writes harmless), skips the cycle when a peer holds the lock. New `go-redsync/redsync/v4` dependency, second independent Redis client in `cmd/route-manager/main.go` (cachehandler's connection isn't exposed to reuse). Deployed cleanly with no orphan container; 2/2 containers healthy, queue consumer=2, dns_sd targets=2, no active ReplicaDegraded alert. Production logs confirm the lock genuinely mutual-excludes: one replica ran the probe cycle ("checked: 2, elapsed: 20s") while the other logged "Another replica already holds the healthcheck lock. Skipping this cycle." on every tick |
| contact | **done** | PR #1245/#177, live-verified 2026-09-02. GetOrCreate/peerlock confirmed dead code (VOIP-1439); standard scale pattern applied, no code change needed. Deployed cleanly with no orphan container; 2/2 containers healthy (RestartCount=0), queue consumers=2 (subscribe + request), dns_sd targets=2, no active ReplicaDegraded alert |
