...
| contact | **in progress (VOIP-1438)** | In-process lock decision resolved: `GetOrCreate`'s per-peer-tuple critical section is now guarded by a redsync cross-replica lock (`pkg/casehandler/peerlock.go`'s `tryAcquireRedisLock`) layered on top of (not replacing) the existing in-process `peerLocks` channel semaphore — in-process serializes same-pod goroutines, Redis serializes across pods. Fails open on Redis errors (safe because `insertWithRetry`'s `uq_case_open_peer` unique constraint + re-query-winner logic already makes concurrent INSERTs safe — a different justification than route's blind-UPDATE argument), fails closed (bounded ~1s budget, explicit `context.WithTimeout`) when another replica holds the lock (`ErrTaken`) or the budget is exhausted. New `go-redsync/redsync/v4` dependency (pinned to route-manager's `v4.15.0`), second independent Redis client in `cmd/contact-manager/main.go`. Standard recipe applied to `komodo/docker-compose.yml` (`container_name` removed, `deploy.replicas: 2`). Deploy-time live verification (queue consumer=2 for both `runListen`/`runSubscribe`, dns_sd targets=2, dual-replica peer-tuple contention test) still pending |
