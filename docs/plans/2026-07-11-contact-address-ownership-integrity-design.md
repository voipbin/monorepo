...
| Customer cascade delete (`ContactDeleteByCustomerID`, via `EventCustomerDeleted` — round-20 finding: this fifth deletion entry point was entirely missing from this design) | Same per-contact period closure as `ContactDelete`, applied to every Contact the bulk delete touches — see round-20 addition below |
