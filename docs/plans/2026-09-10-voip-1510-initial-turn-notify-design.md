...
**결론(확정, 2026-09-10 갱신)**: 대표님이 "1(초기 턴 버그) + 2(예방책)를 하나의
워크플로우로"를 승인했으나, B1 구현 지점 조사 결과 해당 경고가 pipecat 라이브러리
내부(_log_missing_function_call)에서 발생해 별도 서비스(Python pipecat runner) 변경이
필요함이 드러났다. 설계의 "B1이 예상보다 커지면 분리 재검토" 조항에 따라 대표님께
보고했고, **B1을 별도 후속 티켓 VOIP-1512로 분리하기로 승인받았다.**

- **본 PR 포함**: 옵션 A(초기 턴 버그, bin-ai-manager)만. 단일 서비스, 단일 논리 변경.
- **별도 티켓 VOIP-1512**: B1(미등록 tool 호출 런타임 가시화, pipecat runner). 서비스
  경계가 다르고 구현 규모가 옵션 A와 독립적이므로 분리.
- **보류(트리거 대기)**: B2. 저장 시 prompt<->tool_names 정합성 경고. 오탐 위험 + 실측
  재발 신호 없음.
