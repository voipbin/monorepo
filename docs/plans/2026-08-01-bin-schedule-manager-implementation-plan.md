# bin-schedule-manager — Implementation Plan (VOIP-1283, Phase 1)

Status: approved (plan review loop: 3 rounds — RC, Approve, Approve; final revision incorporates round-3 non-blocking guidance)
Design: [2026-08-01-bin-schedule-manager-design.md](2026-08-01-bin-schedule-manager-design.md) (approved, 4-round review)
Branch: `VOIP-1283-Add-bin-schedule-manager` (worktree `.worktrees/VOIP-1283-Add-bin-schedule-manager`)
...
Files (all under `bin-schedule-manager/`):
- `go.mod`: module `monorepo/bin-schedule-manager`, go 1.25.3, replace block copied
  from `bin-tag-manager` **including the self-replace**
  (`replace monorepo/bin-schedule-manager => ../bin-schedule-manager` — the
  convention self-replaces, and the `bin-common-handler ↔ bin-schedule-manager`
...
- `internal/config/main.go`: Bootstrap/LoadGlobalConfig/Get (bin-number-manager
  pattern) with standard vars + SCHEDULE_TICK_INTERVAL_SEC(10),
  SCHEDULE_DISPATCH_CONCURRENCY(10), SCHEDULE_EXECUTION_RETENTION_DAYS(90),
  SCHEDULE_BACKUP_DIR(no default), SCHEDULE_BACKUP_RETENTION_COUNT(7).
...
- `models/outline/queuename.go`: add
  `QueueNameScheduleEvent/Request/Subscribe = "bin-manager.schedule-manager.{event,request,subscribe}"`
...
- `models/outline/servicename.go`: add `ServiceNameScheduleManager ServiceName = "schedule-manager"`.
- `pkg/requesthandler/send_request.go`: add `sendRequestSchedule(...)` private sender
  (pattern: `sendRequestNumber`).
- `pkg/requesthandler/schedule_schedule.go` (new): typed client methods
  `ScheduleV1ScheduleCreate/Get/Gets/Update/Delete/Execute`, `ScheduleV1ExecutionGets`
  using schedule-manager **domain models** (per root CLAUDE.md: prefer domain types
  over transport DTOs for new client methods).
- `pkg/requesthandler/main.go`: interface additions + `go generate ./...` mock regen.
- `pkg/requesthandler/schedule_schedule_test.go` (new): wire-shape tests per rpc.md
  §9.6 (queue, uri, method, marshaled body byte-for-byte, mocked sockhandler).
- `bin-common-handler/go.mod`: `require monorepo/bin-schedule-manager` + `replace`.

Verify (Steps 1+2 combined commit): full 5-step workflow in `bin-schedule-manager`
...
- `pkg/dbhandler/schedule.go`: squirrel CRUD (`schedule_schedules` const), cache-aside,
...
- `pkg/dbhandler/execution.go`:
  ...
  `ExecutionCountAll`.
- `scripts/database_scripts_test/table_schedule_schedules.sql`,
  `table_schedule_executions.sql` (repo `table_` prefix convention; sqlite DDL
...
Verify: full 5-step in bin-schedule-manager.
...
- `pkg/dispatchhandler/main.go` (iface `Run(ctx)`, ctor, mockgen), `tick.go`: ticker
  loop (SCHEDULE_TICK_INTERVAL_SEC) → reap → init-uninitialized → list-due → semaphore
...
- `pkg/listenhandler/main.go`: regex routes (design §6 table incl.
  ...
  `schedule_manager` — NOT bin-tag-manager's `agent_manager` copy-paste bug),
...
- `cmd/schedule-manager/main.go`: bin-number-manager Bootstrap pattern; wires
...
- `cmd/schedule-control/main.go`: direct-DB admin CLI — `schedule list|get|enable|
...
alembic -c alembic.ini revision -m "schedule_schedules_create_table"
alembic -c alembic.ini revision -m "schedule_executions_create_table"
alembic -c alembic.ini revision -m "schedule_schedules_seed_platform_jobs"
...
- `bin-schedule-manager/Dockerfile`: build stage
...
- `k8s/`: `namespace.yml`; `deployment.yml` (replicas: 2, image placeholder
  `schedule-manager-image`, `command: ['./schedule-manager']`, prometheus
  ...
  SCHEDULE_* vars the config actually reads; backup volume/PVC intentionally NOT
...
- `.circleci/config.yml`: mapping line
  `bin-schedule-manager/.*  run-bin-schedule-manager true` (alphabetical).
- `.circleci/config_work.yml`: `run-bin-schedule-manager` parameter; workflow block
  (`build-approval` → `-test` → `-build` → `-release`); jobs with
  `source-directory: bin-schedule-manager` (test), `project-repository:
  bin-schedule-manager` (build), `image-name: schedule-manager` (release) — full
...
- Monorepo doc-sync (ordered):
  1. `bash docs/reference/service-taxonomy-gen.sh > docs/reference/service-taxonomy.md`
     ...
     before extractor).
  2. `bash docs/reference/extractor.sh bin-schedule-manager` — writes extraction JSON
  ...
  3. `docs/reference/rabbitmq-queues-reference.md`: three new schedule-manager queues.
  4. `docs/conventions/database.md` §7.0 prefix table: add `schedule_` row.
  5. `docs/architecture/service-dependency-graph.md`: add schedule-manager node +
...
`kubectl kustomize bin-schedule-manager/k8s` to prove both kustomizations still build.

## Step 8 — go.mod replace propagation + full verification

- Add `require monorepo/bin-schedule-manager v0.0.0-...` + `replace ... =>
  ../bin-schedule-manager` to **every** module that requires `bin-common-handler`:
  `grep -rl "monorepo/bin-common-handler" --include=go.mod .` (36 modules; script the
  edit, **excluding** `bin-common-handler` and `bin-schedule-manager` themselves —
...
- Commit/PR body lists affected projects: `bin-schedule-manager:` (new service),
  `bin-common-handler:` (queues/servicename/client methods),
  `bin-number-manager:` (CronJob removal), `bin-dbscheme-manager:` (migrations),
  `monorepo:`-level docs, `.circleci`. PR title `VOIP-1283-Add-bin-schedule-manager`.
