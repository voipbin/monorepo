				...
				if tt.flowCreateErr == nil && tt.transferType == transfer.TypeBlind {
					...
					if tt.flagAddErr == nil {
						...
						if tt.callHangupErr == nil {
							...
							mockReq.EXPECT().CallV1GroupcallCreate(ctx, uuid.Nil, tt.transfererCall.CustomerID, tt.flow.ID, tt.transfererCall.Source, tt.transfereeAddresses, gomock.Any(), uuid.Nil, cmgroupcall.RingMethodRingAll, cmgroupcall.AnswerMethodHangupOthers, gomock.Any()).Return(tt.groupcall, tt.groupcallCreateErr)
