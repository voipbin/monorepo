...
// Test_topicPatterns_golden pins the EXACT bind set of transfer-manager on the global
...
func Test_topicPatterns_golden(t *testing.T) {
	expected := []string{
		...
		"call-manager.groupcall.*.hangup",
		"call-manager.call.*.hangup",
	}

	// design §5: transfer-manager binds exactly 3 patterns.
...
// Test_fanoutUnbindTargets_golden pins the fanout exchanges transfer-manager unbinds
// after a fully successful topic bind (VOIP-1406). This is the FULL subscribeTargets set
// wired in cmd/transfer-manager: the call-manager event exchange only.
