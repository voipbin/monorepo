...
func Test_errorResponse_typed(t *testing.T) {
	resp := errorResponse(cerrors.NotFound(commonoutline.ServiceNameWebhookManager, "WEBHOOK_NOT_FOUND", "x"))
...
func Test_errorResponse_typedThroughPkgErrorsWrap(t *testing.T) {
	if errorResponse(pkgerrors.Wrap(cerrors.NotFound(commonoutline.ServiceNameWebhookManager, "x", "x"), "outer")).StatusCode != http.StatusNotFound {
...
func Test_errorResponse_typedNotFoundFromBusinessHandler(t *testing.T) {
	typed := cerrors.NotFound(commonoutline.ServiceNameWebhookManager, "WEBHOOK_NOT_FOUND", "x").Wrap(dbhandler.ErrNotFound)
