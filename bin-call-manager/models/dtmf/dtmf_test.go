...
// §2.2). The assertion pins the POINTER receiver: notifyhandler asserts on the dynamic type of the
// event data, which is always a pointer, so a value receiver would silently never be picked up.
