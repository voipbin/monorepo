...
//go:generate mockgen -package groupcallhandler -destination ./mock_main.go -source main.go -build_flags=-mod=mod
...
type GroupcallHandler interface {
	...
	List(ctx context.Context, size uint64, token string, filters map[groupcall.Field]any) ([]*groupcall.Groupcall, error)
