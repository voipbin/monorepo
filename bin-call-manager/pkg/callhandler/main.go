...
//go:generate mockgen -package callhandler -destination ./mock_main.go -source main.go -build_flags=-mod=mod
...
type CallHandler interface {
	...
	List(ctx context.Context, size uint64, token string, filters map[call.Field]any) ([]*call.Call, error)
	...
	Talk(ctx context.Context, callID uuid.UUID, runNext bool, text string, language string, provider string, voiceID string) error
