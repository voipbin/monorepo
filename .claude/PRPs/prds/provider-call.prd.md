...
| 1 | `bin-call-manager` source-validation bypass | New `MetadataKeySkipSourceValidation` constant, add to `ValidMetadataKeys` registry, early-return in `getValidatedSourceForOutgoingCall` when set | in-progress | with 2 | 0 | [`provider-call-skip-source-validation.plan.md`](../plans/provider-call-skip-source-validation.plan.md) |
